...
% Pin the system DejaVu (v2.37) by path: TeX Live/TinyTeX ships an
% older DejaVu (v2.34) that lacks some glyphs used in the Lean
% listings (e.g. the divisibility bar U+2223).
...
with the number systems built from scratch in Lean~4\par}
\vfill
{\small\color{white!60!navyblue}Core Lean first, Mathlib after: build it once by hand, then wield the industrial version.\par}
\vspace{0.5cm}
...
This book rebuilds calculus from a single inductive type in Lean~4 to a working command-line symbolic solver. It proceeds in two deliberate phases.

\medskip\noindent\textbf{Phase one (Part~I): core Lean, no libraries.} No external mathematics libraries are imported. Every definition---addition on the natural numbers, the integers and rationals as quotients, the real numbers as a Cauchy completion---is constructed explicitly and its properties proved by hand. The point of this phase is that you should see, once in your life, that there is no magic underneath: the real numbers can be built, keystroke by keystroke, from \li{zero} and \li{succ}.

\medskip\noindent\textbf{Phase two (Parts~II--VII): Mathlib.} From Chapter~6 onward the book switches---openly, and by design---to \textbf{Mathlib}, the Lean community's mathematical library. Having built $\R$ once yourself, you will use the industrial-strength version of it (Mathlib's \li{Real}) and Mathlib's proof tactics (\li{linarith}, \li{norm\_num}, \li{positivity}, \ldots) for the analysis that follows. This mirrors real practice: nobody re-proves commutativity of addition before doing analysis, but everybody is better at analysis for having done it once. Chapter~6 marks the switch explicitly and shows how to set up a Lean project that depends on Mathlib. The capstone solver of Part~VII is again plain core Lean (it computes over \li{Float}), except for its correctness theorems in Chapter~36, which use Mathlib's analysis library.

Every Lean listing in this book is machine-checked: the Part~I and capstone listings against a bare Lean toolchain (\texttt{CalculusVerification.lean}), the Mathlib-based listings against Mathlib (\texttt{CalculusMathlibVerification.lean}). Where a listing deliberately leaves a proof as \li{sorry}, the surrounding text says so and marks it as an exercise.

\medskip\noindent\textbf{Prerequisites.} Familiarity with proof-based mathematics (induction, contradiction, epsilon-delta arguments seen before). No prior Lean~4 experience required: Appendix~\ref{app:primer} is a primer on every Lean construct and tactic used in this book, including what several of them desugar to under the hood.

\medskip\noindent\textbf{Structure.}
\begin{itemize}[noitemsep]
  \item \textbf{Part~I} constructs $\N$, $\Z$, $\Q$, and $\R$ via Cauchy completion (core Lean, no libraries).
  \item \textbf{Part~II} develops the topology of $\R$: sequences, limits, continuity (from here on, over Mathlib's \li{Real}).
...
If the listing below is your first encounter with Lean~4, do not worry about the syntax: Appendix~\ref{app:primer} (\emph{A Lean 4 Primer}) explains every keyword and tactic this book uses --- \li{inductive}, \li{def}, \li{theorem}, \li{induction \ldots with}, \li{simp}, \li{exact}, and the rest --- with small worked examples and, where illuminating, the lower-level term each tactic expands to.  Read it alongside this chapter, or dip into it whenever a new construct appears.
...
The definitions below may look strange at first --- why define $n + S(m)$ rather than $S(n) + m$?  Recursing on the \emph{second} argument is a convention (it matches Peano's original definition and the one Lean's own \li{Nat} uses); recursing on the first would work just as well.  What matters for termination is only that each recursive call peels one \li{succ} constructor off a designated argument, so the recursion is structurally decreasing.  The commutativity $m + n = n + m$ is then a non-trivial \emph{theorem} that requires induction to prove, not something built into the definition.
...
Formalising this is our first taste of a recurring reality: before Lean will even let us \emph{state} $k < n$ for our own \li{MyNat}, we must define the order and register it with the \li{LE}/\li{LT} type classes, and before the equivalence proofs go through we need a small toolbox of order lemmas ($k \leq 0 \Rightarrow k = 0$, nothing is below zero, $k < n{+}1 \Rightarrow k \leq n$, \ldots).  None of this exists until we build it --- \li{MyNat} is ours alone, so Lean's lemmas about its built-in \li{Nat} do not apply.

\begin{leanbox}
\begin{lstlisting}
-- The order itself: m <= n iff some k witnesses the gap.
def MyNat.le (m n : MyNat) : Prop := ∃ k : MyNat, m.add k = n
def MyNat.lt (m n : MyNat) : Prop := m.le n ∧ m ≠ n

-- Register with the LE/LT type classes so <= and < notation work.
instance : LE MyNat := ⟨MyNat.le⟩
instance : LT MyNat := ⟨MyNat.lt⟩

theorem MyNat.le_refl (n : MyNat) : n ≤ n := ⟨.zero, rfl⟩

-- Key lemma: n + k.succ ≠ n (can't add a positive and return to
-- start). The succ case needs succ_add in the simp set to line the
-- goal up with the induction hypothesis.
...
With the order in place we can build the toolbox and derive strong induction from weak induction by the $Q(n)$ trick from the proof above.

\begin{leanbox}
\begin{lstlisting}
-- The order toolbox: four small lemmas the big proofs lean on.
...
Finally, well-ordering.  The classical proof --- ``pick any witness; either it is minimal or something smaller is in $S$, and descend'' --- becomes a strong induction on the witness.  Totality of the order (\li{lt\_or\_ge}) supplies the ``something smaller'' step, and \li{Classical.em} performs the case split on whether the current witness is already minimal.
...
-- Totality: any two naturals are comparable.
...
-- Strong induction on the witness: either n is already minimal, or
-- S contains some k < n and we recurse on k.
theorem MyNat.well_ordering (S : MyNat -> Prop) (hne : ∃ n, S n) :
    ∃ m, S m ∧ ∀ k, S k -> m ≤ k := by
  obtain ⟨n, hn⟩ := hne
  refine MyNat.strong_ind
    (fun n => S n → ∃ m, S m ∧ ∀ k, S k → m ≤ k) ?_ n hn
  intro n ih hSn
...
The subtle point is \textbf{well-definedness}.  When we write $[(a,b)] + [(c,d)] := [(a+c, b+d)]$, we are defining addition on \emph{equivalence classes} by choosing \emph{representatives}.  This is only legitimate if swapping in a different representative for $[(a,b)]$ or $[(c,d)]$ gives the same equivalence class on the right-hand side.  Checking well-definedness is not optional: it is the mathematical price of the quotient construction, and it must be paid for every operation we define.  In Lean~4 this is enforced by the type system via \li{Quot.lift}, which demands a proof of well-definedness before it will build the lifted function.  For a \emph{binary} operation there is no ready-made \li{Quot.lift2} in core Lean: we lift one argument at a time with two nested \li{Quot.lift}s, and we owe Lean \emph{two} well-definedness proofs --- one for each argument.  Chapter~5 carries this out in full for addition on the reals.

A \emph{commutative} ring with $1 \neq 0$ satisfying $ab = 0 \Rightarrow a = 0$ or $b = 0$ is called an \textbf{integral domain}.  The last property says there are no ``zero divisors'' --- no two nonzero elements whose product is zero.  $\Z$ is an integral domain; this fact will be used in the transitivity proof for $\approx$ in Chapter~3, and it is what allows us to cancel a common nonzero factor.
...
-- The midpoint of two rationals. The positivity proof is assembled
-- by hand from Int.mul_pos (in Part II, Mathlib's `positivity`
-- tactic will discharge goals like this in one line).
...
\begin{leanbox}
\begin{lstlisting}
-- If n*n is even, n is even: split on n mod 2. If n were odd,
-- n*n mod 2 = 1, contradicting 2 ∣ n*n. All arithmetic by omega.
...
theorem sqrt2_irrational :
  ...
  -- p^2 = 2q^2, so p is even; write p = 2k
  have h2p : 2 ∣ p := even_of_sq_even p ⟨q * q, heq⟩
  obtain ⟨k, rfl⟩ := h2p
  -- Then 4k^2 = 2q^2, so q^2 = 2k^2, making q even too
  ...
  -- But then 2 divides gcd(p, q) = 1 -- contradiction
...
\begin{proof}
The Newton sequence above is Cauchy (the error is roughly \emph{squared} at each step --- quadratic convergence) and its limit satisfies $L^2 = 2$, which has no rational solution.
\end{proof}

\begin{remarkbox}
Cauchy completion generalizes: given any \emph{metric space} --- a set equipped with a distance function $d(x,y)$ that is symmetric, vanishes exactly when $x = y$, and satisfies the triangle inequality ($\R$ with $d(x,y) = |x-y|$ is the prototype) --- we can form its completion by the same quotient of Cauchy sequences. This construction is used to define $p$-adic numbers, $L^2$ spaces, and completions of normed rings.
\end{remarkbox}
\color{black}
...
One housekeeping matter before the code: we are still in phase one --- core Lean, no libraries --- and core Lean provides the type \li{Rat} but \emph{no} absolute value on it, and no $|\cdot|$ notation.  True to the from-scratch spirit, we roll our own, \li{rabs}, together with the two facts the construction below actually uses (the remaining standard properties --- $|ab| = |a||b|$, $|a - b| = |b - a|$, and so on --- make good exercises).  The proofs lean on \li{grind}, core Lean's general-purpose arithmetic closer.

\begin{leanbox}
\begin{lstlisting}
-- Absolute value on Rat, from scratch (core Lean has none).
def rabs (q : Rat) : Rat := if q < 0 then -q else q

theorem rabs_zero : rabs 0 = 0 := by simp [rabs]

-- The triangle inequality -- the workhorse of all of analysis.
...
-- Addition of Cauchy sequences: the eps/2 argument from the
-- theorem above, verbatim.
...
Now the payoff of the well-definedness discussion from Chapter~2.  Addition takes \emph{two} \li{MyReal} arguments, so we must lift through the quotient twice --- one nested \li{Quot.lift} per argument --- and prove that \li{addCauchy} respects \li{CauchyEquiv} \emph{in each argument separately}.  Neither proof is optional: omit one and Lean rejects the definition.

\begin{leanbox}
\begin{lstlisting}
-- Respecting the equivalence in the right argument...
theorem addCauchy_respects_right (a b b' : CauchySeq)
    (h : CauchyEquiv b b') :
    CauchyEquiv (addCauchy a b) (addCauchy a b') := by
  intro eps heps
...
-- Addition on MyReal: nested Quot.lift, one well-definedness
-- proof per argument.
...
\section{From \texorpdfstring{\li{MyReal}}{MyReal} to Mathlib}\label{sec:mathlib-switch}

Before the first definition of this chapter, an important and deliberate change of gear.

\textbf{Retiring \li{MyReal}.}  The \li{MyReal} we built in Chapter~5 has served its purpose.  You have now seen, in full detail, that the real numbers are not handed down from anywhere: they can be constructed --- pairs, fractions, Cauchy sequences, three quotients --- from \li{zero} and \li{succ}, with every well-definedness debt paid.  But pushing \emph{analysis} through that construction would mean months of API-building: every $|{\cdot}|$ estimate, every field identity, every completeness argument re-proved by hand before we could even state the Intermediate Value Theorem.  That work has already been done, once, by a community of hundreds --- so from this chapter onward we retire \li{MyReal} and write \li{Real} to mean \textbf{Mathlib's} real numbers.  This is not a retreat: Mathlib's \li{Real} is \emph{the same construction you just carried out} --- equivalence classes of Cauchy sequences of rationals (see \li{Mathlib.Data.Real.Basic}; the quotient lives in \li{CauSeq.Completion}) --- built with more generality and a vastly larger lemma library.  You built the engine once by hand; now you get to drive the production model.

\textbf{What Mathlib is.}  Mathlib is the Lean community's monolithic mathematics library: over a million lines of machine-checked mathematics, from linear algebra through measure theory to schemes, maintained at \url{https://github.com/leanprover-community/mathlib4}.  Importing it gives us two things.  First, \emph{definitions and theorems}: \li{Real}, \li{Real.sqrt}, \li{Real.exp}, \li{Real.sin}, the absolute-value notation $|x|$, \li{Finset} sums, and thousands of named lemmas (\li{abs\_add\_le}, \li{abs\_mul}, \ldots).  Second, \emph{tactics}: powerful proof-automation such as \li{linarith} (linear arithmetic), \li{nlinarith}, \li{norm\_num}, \li{positivity}, \li{ring}, \li{field\_simp}, and \li{push\_neg}, none of which exist in core Lean.  Each is described in Appendix~\ref{app:primer} the first time we use it.

\textbf{Setting up a Mathlib project.}  Lake, Lean's build tool, fetches Mathlib as a dependency.  Create a project directory containing a file \li{lean-toolchain} with the single line \li{leanprover/lean4:v4.31.0} and a \li{lakefile.toml} like this:

\begin{leanbox}[title={lakefile.toml}]
\begin{lstlisting}
name = "verifiedcalculus"
defaultTargets = ["VerifiedCalculus"]
...
Then run \li{lake update} to resolve the dependency and \li{lake exe cache get} to download prebuilt binaries --- do not skip this step, or your machine will spend hours compiling Mathlib from source.  All fetched packages and build artifacts (including the compiled \li{.olean} files) live under the \li{.lake/} directory of your project.  Start your Lean files with \li{import Mathlib} and everything in this chapter compiles.

\textbf{The ground rules from here on.}  We still \emph{define our own} analysis: \li{SeqLimit}, \li{FuncLimit}, \li{ContinuousAt}, \li{HasDerivAt} below are ours, written out in $\varepsilon$--$\delta$ form, not Mathlib's versions --- the point of the book is to build calculus, not to import it.  What we take from Mathlib is the substrate: the real numbers and their ordered-field API, plus the tactics.  (One exception is flagged loudly when it happens: in Chapter~16 we state the Fundamental Theorem against Mathlib's own integral.)  When you have finished the book, comparing our definitions with Mathlib's \li{Filter}-based ones is a highly instructive exercise.
...
-- Limits are unique. The proof mirrors the prose exactly:
-- eps = |L - M|/2, pick an index beyond both thresholds, and let
-- the triangle inequality produce |L - M| < |L - M|.
theorem limit_unique (a : Nat -> Real) (L M : Real)
  ...
  have key : |L - M|
      ≤ |a (max N1 N2 + 1) - L| + |a (max N1 N2 + 1) - M| := by
    calc |L - M|
        = |(L - a (max N1 N2 + 1)) + (a (max N1 N2 + 1) - M)| := by
          ring_nf
      _ ≤ |L - a (max N1 N2 + 1)| + |a (max N1 N2 + 1) - M| :=
          abs_add_le _ _
...
Three Mathlib tactics make their debut here: \li{by\_contra} turns the goal $L = M$ into a hypothesis $L \neq M$ with goal \li{False}; \li{positivity} dispatches the ``obviously positive'' side-goal; and \li{linarith} finishes by deriving a contradiction from the linear inequalities in scope ($|L-M| \leq h_1 + h_2$, $h_1 < |L-M|/2$, $h_2 < |L-M|/2$).  Appendix~\ref{app:primer} covers all three.
...
-- |x^2 - 4| = |x+2||x-2|.  If |x-2| < 1 then |x+2| < 5.
...
example : FuncLimit (fun x => x * x) 2 4 := by
  ...
  calc |x * x - 4| = |x - 2| * |x + 2| := by rw [← abs_mul]; ring_nf
    _ < (eps / 5) * 5 :=
        mul_lt_mul'' hx5 hbound (abs_nonneg _) (abs_nonneg _)
    _ = eps := by ring
\end{lstlisting}
\end{leanbox}
\color{black}
...
\textbf{Fourier} (1822) showed that a discontinuous function --- one with jumps --- could be represented as an infinite sum of continuous sine and cosine functions. This raised an immediate question: if the \emph{limit} of continuous functions is not necessarily continuous, what exactly does ``continuous'' mean? Cauchy claimed (incorrectly) that the \emph{pointwise} limit of continuous functions is continuous. \textbf{Abel} found a counterexample in 1826; \textbf{Seidel} and \textbf{Stokes} independently clarified the issue in 1847--48 by distinguishing uniform from pointwise convergence: it is the \emph{uniform} limit of continuous functions that is guaranteed continuous (the uniform-limit theorem), while pointwise limits can jump.
...
theorem sq_continuous_at (a : Real) : ContinuousAt (fun x => x * x) a := by
  intro eps heps
  -- |x^2 - a^2| = |x-a||x+a|. If |x-a| < 1, |x+a| <= |x-a| + 2|a| < 2|a| + 1
...
For any $y > 0$ and any integer $n \geq 1$, $y^{1/n}$ exists as a real number.
\end{corbox}
\color{black}
...
\begin{proof}
$(\Rightarrow)$: Compact sets are closed and bounded (the argument works in any metric space --- any set with a distance function, of which $\R$ with $d(x,y) = |x-y|$ is the basic example). Boundedness: the cover $\{(-n,n)\}_{n\geq 1}$ has a finite subcover. Closure: if $x$ is a limit point not in $K$, the sets $\R \setminus \{y : |y-x| \leq 1/n\}$ cover $K$ but no finite subcollection does.
...
A set $S \subseteq \R$ is \textbf{connected} if it cannot be written as a disjoint union $S = A \cup B$ of two nonempty sets that are \emph{open in $S$} --- that is, $A = S \cap U$ and $B = S \cap V$ for open sets $U, V \subseteq \R$ (this is the \textbf{subspace topology} on $S$). The relativisation matters: $S = [0,1] \cup [2,3]$ is visibly disconnected, yet neither piece is open in $\R$; each \emph{is} open in $S$ (take $U = (-1, 1.5)$ and $V = (1.5, 4)$). The connected subsets of $\R$ are exactly the intervals (including rays and $\R$ itself).
\end{defbox}
\color{black}
...
\begin{proof}
Suppose $f(S) = A \cup B$ with $A, B$ nonempty, disjoint, and open in $f(S)$, say $A = f(S) \cap U$ and $B = f(S) \cap V$ with $U, V$ open. Then $S = (S \cap f^{-1}(U)) \cup (S \cap f^{-1}(V))$, a disjoint union of nonempty sets open in $S$ (preimages of open sets are open, by continuity of $f$). This contradicts the connectedness of $S$.
\end{proof}
...
-- The derivative of a constant is zero: the difference quotient
-- (c - c)/h is literally 0, so simp closes everything.
theorem const_deriv (c x : Real) : HasDerivAt (fun _ => c) x 0 := by
  intro eps heps
  refine ⟨1, one_pos, fun h _ _ => ?_⟩
  simpa using heps

-- The derivative of x is 1: the quotient is h/h = 1 (h ≠ 0!).
...
\begin{remarkbox}
\textbf{An honest accounting: where do $\sin$, $\cos$, and $e^x$ come from?}

Notice that the last two proofs \emph{consume} facts we have not yet established: the angle-addition formula, the limits $\sin(h)/h \to 1$ and $(e^h - 1)/h \to 1$, indeed the very definitions of $\sin$, $\cos$, and $e^x$.  This book does not construct these functions from first principles.  The standard rigorous route --- which we sketch but do not complete --- is to \emph{define} them by their power series ($e^x = \sum x^n/n!$, etc.), prove the series converge everywhere (Chapter~20), and derive the derivative rules by term-by-term differentiation (Chapters~20--21).  In Lean, this is exactly what Mathlib does: \li{Real.exp}, \li{Real.sin}, and \li{Real.cos} are defined via power series (over $\mathbb{C}$, then restricted), and the rules above are library theorems (\li{Real.hasDerivAt\_sin} and friends).  When the capstone's differentiation engine applies $(\sin u)' = \cos(u)\,u'$, it is these theorems that back the rule.
\end{remarkbox}
\color{black}
...
\begin{leanbox}
\begin{lstlisting}
-- Newton's method as a recursive function. Note the type: Float,
-- not Real. Mathlib's Real is noncomputable (a Real is an
-- equivalence class of Cauchy sequences -- there is nothing to
-- "run"), so a Real-valued newton could be defined but never
-- #eval'd. Float is the computational stand-in.
...
#eval sqrtNewton 5 1.5  -- prints 1.414214 (Float's default precision)
\end{lstlisting}
\end{leanbox}
\color{black}
...
For $f : [a,b] \to \R$ \textbf{bounded} (boundedness is what guarantees the suprema and infima below exist), the \textbf{upper Darboux sum} and \textbf{lower Darboux sum} are:
...
A \textbf{bounded} $f$ is \textbf{Riemann integrable} on $[a,b]$ if:
...
-- Upper and lower Darboux sums (sketch: the two sorries stand for
-- the sup and inf of f on a subinterval, which exist because f is
-- bounded; constructing them is this chapter's project exercise).
noncomputable def upperSum {a b : Real} (f : Real -> Real)
    (P : Partition a b) : Real :=
  (subintervals P).foldl (fun acc (p : Real × Real) =>
    let Mi : Real := sorry  -- sup of f on [p.1, p.2]
    acc + Mi * (p.2 - p.1)) 0

noncomputable def lowerSum {a b : Real} (f : Real -> Real)
    (P : Partition a b) : Real :=
  (subintervals P).foldl (fun acc (p : Real × Real) =>
    let mi : Real := sorry  -- inf of f on [p.1, p.2]
    acc + mi * (p.2 - p.1)) 0
...
\begin{enumerate}
  ...
  \item Complete the formalization of \li{IsIntegrable} over Mathlib's \li{Real} type: replace the two \li{sorry}s in the sketch above with genuine suprema and infima (Mathlib's \li{sSup}/\li{sInf} of \li{f '' Set.Icc p.1 p.2}), adding the boundedness hypothesis the definition needs.
\end{enumerate}
\end{exbox}
\color{black}
...
\begin{proof}
Let $F(x) = \int_a^x f(t)\,dt$. By Part 1, $F' = f = G'$, so $G - F$ is constant (by MVT, $(G-F)'=0 \Rightarrow G-F=c$). Evaluating the constant at $x = a$: $c = G(a) - F(a) = G(a)$, since $F(a) = 0$. Hence $F(x) = G(x) - G(a)$ for all $x$, and in particular $\int_a^b f = F(b) = G(b) - G(a)$.
\end{proof}
...
Formalising FTC honestly requires an \emph{actual integral}, and our Chapter~15 sketch stopped short of one (its Darboux sums still contain \li{sorry}s for the suprema).  Rather than state a theorem about an integral we never finished defining, this is the one place in the book where we lean on Mathlib's analysis library itself: Mathlib's interval integral \li{∫ x in a..b, f x}, its own derivative predicate (written \li{\_root\_.HasDerivAt} below to distinguish it from our $\varepsilon$--$\delta$ version --- the underscore prefix says ``the top-level one, not ours''), and its integrability predicate \li{IntervalIntegrable}.  The reward for stating it in Mathlib's terms: the proof is a \emph{one-liner}, because Mathlib has already done the work of this whole chapter, in full generality.

\begin{leanbox}
\begin{lstlisting}
-- FTC Part 2, stated against Mathlib's interval integral and
-- proved by Mathlib's own FTC. The hypothesis asks G to have
-- derivative f(x) at every x between a and b (Set.uIcc a b is
-- the closed interval between them, whichever is larger).
...
Completing the story with our own definitions --- proving that a continuous $f$ satisfies \li{IsIntegrable} (Chapter~15) and that the resulting value equals $G(b) - G(a)$, following the paper proof above --- is a substantial but rewarding project; the sketch in the paper proof is the roadmap.
...
The radius-of-convergence formula below uses one piece of vocabulary we have not yet defined: the \emph{limit superior}.  A bounded sequence need not converge, but the suprema of its tails always settle down.

\begin{defbox}{Limit Superior}
For a sequence $(a_n)$ of reals, the \textbf{limit superior} is
\[ \limsup_{n\to\infty} a_n := \lim_{n\to\infty}\, \Bigl(\sup_{k \geq n} a_k\Bigr), \]
i.e.\ the limit of the tail-suprema $s_n = \sup\{a_k : k \geq n\}$.  The $s_n$ are non-increasing, so by the Monotone Convergence Theorem the limit always exists when $(a_n)$ is bounded above (we set $\limsup a_n = +\infty$ otherwise, and $-\infty$ if $a_n \to -\infty$).  Equivalently, $\limsup a_n$ is the largest limit of any convergent subsequence.  For a convergent sequence, $\limsup a_n = \lim a_n$; the point of $\limsup$ is that it exists even when the plain limit does not --- e.g.\ $\limsup\,(-1)^n = 1$.
\end{defbox}
\color{black}
...
Its \textbf{radius of convergence} is $R = 1/\limsup_{n\to\infty} |c_n|^{1/n}$ (the Cauchy--Hadamard formula, with the conventions $1/0 = \infty$ and $1/\infty = 0$). The series converges absolutely for $|x-a| < R$ and diverges for $|x-a| > R$.
\end{defbox}
\color{black}
...
-- Taylor polynomials for e^x (noncomputable: Real division is)
...
theorem expTaylor_converges (x : Real) :
  ...
  sorry  -- exercise: series convergence machinery from Ch. 19-20

-- e^x satisfies (e^x)' = e^x (our epsilon-delta HasDerivAt)
theorem exp_deriv (x : Real) : HasDerivAt Real.exp x (Real.exp x) := by
  sorry  -- exercise: term-by-term differentiation, or transfer
         -- Mathlib's Real.hasDerivAt_exp to our definition
\end{lstlisting}
\end{leanbox}
\color{black}
...
theorem cauchy_schwarz {n : Nat} (v w : Vec n) :
  ...
  sorry  -- exercise: expand normSq and apply the discriminant argument
\end{lstlisting}
\end{leanbox}
\color{black}
...
\begin{leanbox}
\begin{lstlisting}
-- Partial derivative: fix all coordinates except the i-th and
-- take the one-variable derivative of the resulting function.
-- Here we use Mathlib's deriv (which returns 0 when the function
-- is not differentiable -- a "junk value", a standard Mathlib
-- idiom that keeps partialDeriv total).
...
so $\lambda=\pm 1$.  Taking $\lambda = 1$ gives $y = x/2$; substituting into the constraint, $x^2/4 + x^2/4 = 1$, so $x = \sqrt{2}$, $y = 1/\sqrt{2}$.  The maximum is $f = \sqrt{2}\cdot\tfrac{1}{\sqrt{2}} = 1$, attained at $(\sqrt{2},1/\sqrt{2})$ (and at $(-\sqrt{2},-1/\sqrt{2})$).
\end{example}
...
The previous six parts built the real numbers from scratch and then developed
calculus over them, proving its core theorems.
...
step-by-step traces --- just like Symbolab, but with every rule
traced back to the mathematics we proved.  The capstone is once again plain
core Lean --- it computes over \li{Float} and needs no Mathlib to build or
run; only the correctness theorems of Chapter~36 bring Mathlib back.
...
Once we have the syntax tree, evaluation gives it meaning: given an assignment of numerical values to variables, \li{eval} traverses the tree and computes the corresponding \li{Float}.  This is the \textbf{denotational semantics} of our expression language --- the same idea by which programming-language theory assigns mathematical meaning to programs, here applied to a concrete arithmetic language.  Note that \li{eval} is total: every well-formed \li{Expr} has a value (though it may be \li{Float.nan} or \li{Float.inf} for divisions by zero).
...
def Expr.pp : Expr -> String
  | Const c     => if c == Float.floor c then
                     toString c.toInt64   -- print 2.0 as "2"
                   else toString c
  | Var x       => x
...
One Lean subtlety shapes the code: you cannot pattern-match on \li{Float}
\emph{literals}.  A pattern like \li{Add e (Const 0)} is rejected, because
\li{0 : Float} is not a constructor --- it elaborates to a function call
(\li{Float.ofScientific \ldots}), and match patterns must be built from
constructors.  So the identity rules match \li{Const c} and test
\li{c == 0} with an ordinary \li{if} guard on the right-hand side.
...
A syntax note: because \li{diff} is not declared inside the \li{Expr} namespace, the \emph{patterns} use the dot-prefixed constructor names (\li{.Add}, \li{.Mul}, \ldots), which Lean resolves against the type being matched.  Writing the bare names under an \li{open Expr} would collide with the arithmetic type classes \li{Add}, \li{Mul}, \li{Pow}, \li{Neg} that core Lean defines --- Lean rejects such patterns as ambiguous.  On the right-hand sides, where the expected type disambiguates, the bare names (via \li{open Expr}) are fine.

\begin{leanbox}
\begin{lstlisting}
open Expr

-- diff x e returns (derivative, trace)
def diff (x : String) : Expr -> Expr × DiffStep
  ...
  -- note the trace is the LEAF .PowerRule -- the inner derivation su
  -- is used for the derivative but not recorded in the trace)
  | .Pow u (Const n) =>
      let (u', su) := diff x u
...
simplifies the integrand.  It needs three small syntactic helpers first: the list of
variables occurring in an expression, and two substitution functions (rename a
variable, and replace a variable by an expression).  Each is the same mechanical
structural recursion over the fourteen constructors.
Note also \li{innerFunctions}: a \li{Pow} node contributes only its \emph{base} as a
candidate --- a second alternative \li{Pow \_ u} would be rejected by Lean as
redundant, since the first pattern already matches every \li{Pow}.
...
def varsOf : Expr -> List String
  ...
  | .Add a b | .Sub a b | .Mul a b | .Div a b | .Pow a b =>
      varsOf a ++ varsOf b
  | .Neg a | .Sin a | .Cos a | .Tan a | .Exp a | .Ln a | .Abs a =>
      varsOf a

-- Rename variable x to u throughout
...
-- Replace variable u by the expression repl: identical shape to
-- substVar, with the Var case reading
...
-- "e is expressed in terms of u": every variable of e occurs in u
...
A frank engineering note: this detector is deliberately naive.  Because \li{fullSimplify} only folds constants and identities --- it cannot cancel a symbolic factor $g'(x)$ from a quotient --- \li{tryUSub} fires only on the easiest shapes.  Strengthening the simplifier until $\int 2x\,e^{x^2}\,dx$ goes through is one of this chapter's exercises, and a genuinely instructive one.

\section{Integration by Parts Heuristic}

The LIATE rule (Logarithm, Inverse trig, Algebraic, Trig, Exponential) guides
which factor to assign to $u$ and which to $dv$.  Two implementation notes.
First, our \li{Expr} has no inverse-trig constructors (adding \li{Arctan} and
\li{Arcsin}, with priority~1 here, is Exercise~2 of Chapter~32), so the printed
table skips that tier.  Second, the ``algebraic'' tier must include the bare
variable: $x$ is the polynomial $x^1$, and if \li{Var} fell through to the
default priority it would rank \emph{below} \li{Exp} --- the engine would then
pick $u = e^x$, $dv = x\,dx$ for $\int x e^x\,dx$ and fail.  (In an earlier
draft it did exactly that.  Heuristics deserve tests.)

\begin{leanbox}
\begin{lstlisting}
-- LIATE priority for choosing u (lower = more eager to be u)
def liatePriority : Expr -> Nat
  | .Ln _               => 0   -- highest: logarithm
  | .Pow (Var _) _      => 2   -- algebraic (polynomial x^n)
...
The IBP strategy itself calls the top-level \li{integrate} on the remainder
integral $\int v\,du$ --- so \li{tryIBP} and \li{integrate} are \emph{mutually
recursive}, and the recursion is not structurally decreasing (the remainder is a
freshly built expression, not a subterm).  Lean therefore cannot verify
termination, and we must declare the whole cluster inside a \li{mutual} block
with \li{partial def}.  This is an honest trade: \li{partial} functions can be
compiled and run but are opaque to the proof system, and any claim that the
engine terminates on all inputs is exactly that --- a claim, unproven.  For
well-founded inputs it terminates in practice because each remainder is
strictly simpler; making that precise (and provable) is a hard exercise.
...
Two of the strategies in the dispatch list --- \li{partialFracStrategy} and \li{trigReduceStrategy} --- are left for you to implement (Exercises~2 and~3 below).  So that the engine compiles and runs meanwhile, we declare them as stubs that always decline; they sit \emph{before} the \li{mutual} block, since they are not (yet) recursive.  The remaining strategies continue the \li{mutual} block begun with \li{tryIBP} above --- \li{sumStrategy} and \li{constMulStrategy} also call back into \li{integrate} --- and the block closes with \li{end}.

\begin{leanbox}
\begin{lstlisting}
-- Stubs for the two exercise strategies (place BEFORE the mutual
-- block): always decline, so the dispatcher moves on.
def partialFracStrategy : Strategy := fun _ _ => none
def trigReduceStrategy  : Strategy := fun _ _ => none

-- ... continuing the mutual block from tryIBP:

partial def integrate (x : String) (e : Expr) :
    Option (Expr × IntStep) :=
...
partial def constMulStrategy (x : String) :
    Expr -> Option (Expr × IntStep)
  | .Mul (Const c) e =>
...
-- Convert DiffStep to SolveTrace. The match must be exhaustive:
-- forget the SubRule case and Lean rejects the definition with
-- "missing cases".
def DiffStep.toTrace (e result : Expr) : DiffStep -> SolveTrace
  ...
  | .SubRule s t  =>
      .Node "Difference Rule" result
        [s.toTrace e result, t.toTrace e result]
  | .ProductRule s t =>
...
Observe an honest limitation before we render anything: \li{DiffStep} records which \emph{rules} fired, but not the intermediate \emph{results} --- so \li{toTrace} has nothing better to thread into a compound child node than the top-level \li{result}.  Compound sub-steps will therefore repeat the final expression rather than show their own sub-derivative, and \li{PowerRule}, being a leaf, shows no inner derivation at all.  Upgrading \li{DiffStep} to carry the result of each sub-derivation (and \li{diff}'s \li{Pow} case to keep its inner trace) is Exercise~3 --- the payoff is a trace that reads like Symbolab's.
...
\begin{leanbox}
\begin{lstlisting}
-- Render a SolveTrace as an indented list. "".pushn ' ' n builds
-- the n-space indent; List.flatMap concatenates the per-child
-- line lists.
...
▶ Product Rule → sin(x) + x * cos(x)
  • Variable Rule: 1
  ▶ Chain Rule (sin) → sin(x) + x * cos(x)
    • Variable Rule: 1
\end{verbatim}

The derivative is right, and the rule skeleton is right --- product rule at the root, with the variable rule and a chain-rule-on-$\sin$ beneath it.  The blemish is the one flagged above: the Chain Rule line repeats the final answer instead of showing its own contribution $\cos(x)$, because \li{DiffStep} never recorded it.  Exercise~3 fixes this.
\end{example}
...
\begin{enumerate}
  ...
  \item Run \li{solveIntegral "x" (Mul (Var "x") (Exp (Var "x")))} and verify
        the output is \li{x * exp(x) - exp(x) + C} --- which equals
        $(x-1)e^x + C$, though the engine does not factor.  Bonus: add a
        factoring rule to \li{simplify} so it does.
\end{enumerate}
\end{exbox}
\color{black}
...
Lake is Lean~4's package manager and build system.  A Lake project specifies its modules (libraries) and executables in a \li{lakefile.lean} (or \li{lakefile.toml}, as in the Mathlib setup of Chapter~6) at the project root.  Our project has one library (\li{Symboliculus}, containing the expression, differentiation, integration, and trace modules) and one executable (\li{symboliculus}, the CLI entry point).  Running \li{lake build} compiles everything and places all build artifacts under the \li{.lake/} directory --- the binary lands at \li{.lake/build/bin/symboliculus}.  Note that no Mathlib dependency is declared: the engine is plain core Lean.
...
Below are the canonical invocations for the two main use cases: differentiation with step trace, and integration.  The outputs shown are the engine's \emph{actual} output, so you can diff your build against them character for character.  For the derivative trace, recall the Chapter~34 caveats: compound sub-steps repeat the final expression, and the power rule prints as a leaf (Exercise~3 of Chapter~34 upgrades both).  For the integral, note the engine returns the unfactored \li{x * exp(x) - exp(x)} --- mathematically $(x-1)e^x$ --- and, since \li{IntStep.toTrace} is still Exercise~1 of Chapter~34, the \li{--steps} flag reports only that a trace is available.  Try feeding in more complex expressions; the engine should handle any combination of the functions and rules implemented in the library.
...
--   • Power Rule: 2 * x * sin(x) + x^2 * cos(x)
--   ▶ Chain Rule (sin) → 2 * x * sin(x) + x^2 * cos(x)
--     • Variable Rule: 1
...
The differentiation engine \emph{applies correct rules}: each case of \li{diff}
transcribes a theorem from Part~III.  Can we turn that observation into a
machine-checked theorem about the program?  Mostly yes --- but being precise
about the ``mostly'' is the most instructive part of this section.

\textbf{First obstacle: \li{Float} is not $\R$.}  The tempting statement is
``\li{(diff x e).1.eval env} equals the derivative of \li{fun t => e.eval
(env[x := t])}.''  It is doubly broken.  Formally, \li{eval} produces
\li{Float}, and a derivative (Mathlib's \li{deriv}, or our \li{HasDerivAt})
only makes sense over a normed field --- which \li{Float} is not: it is a
finite set of $2^{64}$ bit patterns with rounding arithmetic (not even
associative: try \li{\#eval (0.1 + 0.2) + 0.3 == 0.1 + (0.2 + 0.3)}).
And substantively, the statement is \emph{false} for Float: the sum rule
$(u+v)' = u' + v'$ holds exactly over $\R$, but Float addition rounds, so no
exact equality of this shape can survive.  Correctness of numerical code is
always a statement about the \emph{ideal semantics}, with rounding handled
separately (that is the subject of numerical analysis).

So we give \li{Expr} a second, real-valued semantics --- reading an expression
as a function $\R \to \R$ of the single variable \li{x} --- and state
correctness against \emph{that}.  The stored \li{Float} constants are read
through a map \li{ι : Float → Real} (think ``the exact rational this bit
pattern denotes''), of which we require only \li{ι 0 = 0} and \li{ι 1 = 1}.
This section uses Mathlib (\li{Real.sin}, \li{HasDerivAt}, and friends);
it is the one place the capstone project needs it.

\begin{leanbox}
\begin{lstlisting}
-- A real-valued semantics for Expr: interpret e as a function of
-- the single variable x. (noncomputable: it lives in Real.)
...
\begin{thmbox}{Correctness of \li{diff} (rule-by-rule, over $\R$)}
Read expressions through \li{evalR}.  Then each rule \li{diff} applies is
correct: wherever the sub-expressions are differentiable (with the derivatives
claimed by the recursive calls), the expression built by the corresponding
\li{diff} case has the claimed derivative.  For the total fragment ---
\li{Const}, \li{Var}, \li{Add}, \li{Sub}, \li{Mul}, \li{Neg}, \li{Sin},
\li{Cos}, \li{Exp} --- this holds unconditionally; for \li{Div}, \li{Pow},
\li{Ln}, and \li{Abs} it holds under the usual side conditions (nonvanishing
denominator, positive base, etc.), which is why a single global equation
cannot be stated without qualifications.
\end{thmbox}
\color{black}

In Lean we prove the two base cases of \li{diff} itself, and the inductive
steps as \emph{rule-level} lemmas (shown here for \li{Add} and \li{Sin}; the
remaining total-fragment cases follow the same two-line pattern and are the
extended exercise below).  Note how much Mathlib is doing for us: \li{hu.add hv}
\emph{is} the sum rule, and \li{hu.sin} \emph{is} the chain rule for sine.
...
-- of expression trees (the rule diff's Add case applies, before its
...
(If you prefer the statement in terms of Mathlib's \li{deriv} function:
\li{HasDerivAt f L t} implies \li{deriv f t = L} --- that is Mathlib's
\li{HasDerivAt.deriv} --- so each lemma above yields the corresponding
\li{deriv} equation.)

\textbf{Second obstacle: the simplifier folds constants in \li{Float}.}
Why lemmas per rule instead of one induction over \li{e}?  Assembling the full
theorem for the \emph{shipped} \li{diff} needs one more ingredient: every
compound case of \li{diff} finishes with \li{fullSimplify}, so we would need
\li{simplify} to preserve \li{evalR}-semantics.  And here honesty bites:
it does not, exactly.  \li{simplify} rewrites \li{Add (Const a) (Const b)}
to \li{Const (a + b)} --- \emph{Float} addition, which rounds.  Over the reals
this rule is only valid if \li{ι (a + b) = ι a + ι b}, and no map from a
$64$-bit type into $\R$ can make rounding arithmetic exact (the identity-style
rules like $e + 0 \to e$ are fine; constant \emph{folding} is not).  This is
not pedantry --- it is precisely the gap between symbolic and floating-point
computation, and formalisation forced us to notice it.

\begin{exbox}[title={Extended Exercise: the full correctness theorem}]
\begin{enumerate}
  \item Prove the remaining total-fragment rule lemmas (\li{Sub}, \li{Mul},
        \li{Neg}, \li{Cos}, \li{Exp}) in the style of
        \li{add\_rule\_correct}, using Mathlib's \li{HasDerivAt.sub},
        \li{.mul}, \li{.neg}, \li{.cos}, \li{.exp}.
  \item Define a simplification-free \li{diffRaw} (drop the
        \li{fullSimplify} calls) and prove by structural induction, from your
        rule lemmas, that \li{diffRaw} is correct on the total fragment.
  \item Restrict \li{simplify} to its semantics-preserving rules (drop the
        four constant-folding cases), prove it preserves \li{evalR}, and
        conclude correctness for the corresponding variant of \li{diff}.
  \item Ponder (or prove!): for the \emph{original} \li{simplify} the
        preservation statement is false.  Exhibit concrete Float constants
        witnessing this.
\end{enumerate}
\end{exbox}
\color{black}
...
\textbf{Robert Risch} solved the general case in 1969--1970 with a complete algorithm: given any elementary function, the Risch algorithm either finds an elementary antiderivative or proves none exists. One fine print matters, though. The algorithm repeatedly needs to decide whether certain expressions are identically zero, and by \textbf{Richardson's theorem} (1968) zero-equivalence for a rich enough class of closed-form expressions (involving $\pi$, $\ln 2$, $e^x$, $\sin x$, $|x|$, \ldots) is \emph{undecidable} in general. So Risch's procedure is an algorithm \emph{modulo a zero-equivalence oracle}: fully decidable on well-behaved differential-field towers (for instance, purely transcendental ones with constant field $\Q$), and heuristic at the ragged edges where constant recognition itself is unsolvable. In practice, a full implementation requires handling dozens of cases across algebraic, logarithmic, and exponential extensions. The most complete implementations, in systems such as Mathematica, Maple, and Axiom, run to tens of thousands of lines --- and even they implement the algebraic case only partially.
...
There is a procedure (the \textbf{Risch algorithm}) that, given an elementary
function $f$ presented over a suitable differential field, either:
\begin{enumerate}[noitemsep]
  \item Computes an elementary antiderivative $F$ with $F'=f$, or
  \item Proves that no elementary antiderivative exists.
\end{enumerate}
The procedure is a genuine decision procedure \emph{relative to an oracle for
deciding whether expressions are identically zero}; by Richardson's theorem
that sub-problem is undecidable for general closed-form expressions, so
``symbolic integration is decidable'' must always carry this asterisk.
\end{thmbox}
\color{black}

\begin{remarkbox}
The Risch algorithm is \emph{extremely} complex
to implement fully --- the most complete implementations (as in Mathematica or Maple) run
to tens of thousands of lines of code and handle dozens of special cases, and none
is complete for the algebraic case.  Our engine
...
\section{Connections to Neighbouring Fields}

Symboliculus is not an isolated project --- every major component opens a door
to a larger subject worth exploring on its own:

\begin{itemize}
  \item \textbf{\li{Expr} and programming-language theory.}  The \li{Expr}
        inductive type is a \emph{term algebra} in the sense of the lambda
        calculus and universal algebra: constructors are introduction rules,
        pattern matching is elimination, and \li{eval} is a denotational
        semantics.  Adding binders, capture-avoiding substitution, and rewrite
        rules turns it into a full term rewriting system --- the gateway to
        PL theory.

  \item \textbf{Strategy dispatch.}  The integration engine's
        strategy chain --- try \li{directLookup}, fall back to \li{tryUSub}, then
        \li{tryIBP}, etc.\ --- is ordered, first-match-wins dispatch over
        \li{Option}, short-circuiting on the first \li{some}.  Serious computer
        algebra systems generalise this to backtracking search over a space of
        rewriting tactics --- the same design space as proof search in Lean
        itself.

  \item \textbf{Correctness proofs and differential Galois theory.}  The field
        of rational functions over which the Risch algorithm operates is a
        \emph{differential field}.  Whether an elementary antiderivative exists
        is answered by Liouville's theorem and, more deeply, by Galois theory
        applied to differential equations (Picard--Vessiot theory) --- a
        beautiful subject if you have met Galois theory for polynomials.

  \item \textbf{Numerical integration.}  When the symbolic engine returns
        \li{none}, real systems fall back to numerical integration (Gaussian
        quadrature, Simpson's rule) --- numerical analysis, the other half of
        the Float discussion from the previous section.

  \item \textbf{Optimisation and the Hessian.}  The multivariable optimisation
        chapter (Ch.\ 26) is the calculus behind machine-learning
        optimisation: gradient descent follows $-\nabla f$, and the Hessian
        controls its convergence speed.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Bridge \li{MyReal} to Mathlib's \li{Real}.}  Part~I's
        \li{MyReal} and Mathlib's \li{Real} arise from the same Cauchy-sequence
        construction.  Make that precise: build the field isomorphism
        \li{MyReal ≃ Real} and use it to transport a Part~I theorem across.
        Then extend Section~36.1's rule lemmas into the full
        \li{diff}-computes-\li{HasDerivAt} bridge.
...
\begin{exbox}
\begin{enumerate}
  \item Work the Extended Exercise of Section~36.1: prove the remaining
        rule-correctness lemmas (\li{Sub}, \li{Mul}, \li{Neg}, \li{Cos},
        \li{Exp}), then tackle the side-condition cases (\li{Div}, \li{Ln},
        \li{Pow}) with appropriate hypotheses.
...
This appendix is the promised zero-prerequisites tour of every Lean~4 construct and tactic used in this book.  Each entry says what the construct does, shows a small self-contained example (every example here is machine-checked), and --- where it illuminates --- shows the lower-level term the tactic is sugar for.  That last part is worth your attention: Lean tactics are not magic, they are \emph{programs that build proof terms}, and seeing the terms once demystifies them permanently.

Entries marked \textbf{(Mathlib)} require \li{import Mathlib} (see the setup instructions in Section~\ref{sec:mathlib-switch}); everything else is core Lean.

\section{Declarations}

\subsection*{\li{inductive} --- defining new types}

An \li{inductive} declaration creates a brand-new type from a list of \emph{constructors} --- the only ways to make values of the type.  The kernel automatically knows that constructors are injective and disjoint, and generates an induction principle (Section~\ref{sec:primer-induction}).
...
inductive Tree where
  | leaf : Tree
  | node : Tree -> Tree -> Tree
  deriving Repr   -- auto-generate printing, so #eval can show Trees
\end{lstlisting}
\end{leanbox}
\color{black}

\li{deriving Repr} asks Lean to synthesise a printer; \li{deriving BEq} (used by the capstone's \li{Expr}) synthesises the boolean equality test \li{==}.

\subsection*{\li{def} and pattern matching}

A \li{def} introduces a function or value.  Functions on inductive types are usually written by \emph{pattern matching}: one equation per constructor.  Recursion is allowed as long as some argument structurally shrinks (Lean checks this; when it cannot, you must write \li{partial def}, as the integration engine of Chapter~33 does, and you give up the ability to reason about the function in proofs).
...
The leading dot in \li{.leaf} means ``the constructor of the type being matched'' --- shorthand for \li{Tree.leaf}.  Because the function is named \li{Tree.size}, the \emph{dot notation} \li{t.size} works for any \li{t : Tree}.

\subsection*{\li{theorem}, \li{example}, and \li{Prop}}

In Lean, propositions are types (of sort \li{Prop}) and proofs are programs: to prove $P$ is to construct a term of type $P$.  A \li{theorem} is a named proof; an \li{example} is an anonymous one (used throughout this appendix).  The body can be a direct \emph{term} or a \emph{tactic block} introduced by \li{by}, in which tactics manipulate the current \emph{goal} until none remain.
...
\subsection*{Type classes and \li{instance}}

A \emph{type class} is an interface: \li{Add α} says ``$\alpha$ has a \li{+}''.  An \li{instance} registers an implementation, and Lean finds it automatically whenever \li{+} is used --- this is how \li{MyNat} gets \li{+} in Chapter~1 and how \li{MyNat.le} becomes \li{≤}.
...
The angle brackets \li{⟨\ldots⟩} are the \emph{anonymous constructor}: Lean infers which constructor to apply from the expected type.  They also build proofs of $\exists$ and $\land$ --- \li{⟨k, hk⟩} proves \li{∃ k, P k} by supplying the witness and the proof.

\subsection*{\li{@[simp]} --- the simplifier's rule set}

The attribute \li{@[simp]} marks an equation as a rewrite rule for the \li{simp} tactic, which rewrites left-to-right until no rule applies.  Marking a \emph{definition} \li{@[simp]} (as Chapter~1 does with \li{MyNat.add}) makes \li{simp} unfold it.
...
\subsection*{\li{rfl} --- true by computation}

Proves any goal of the form \li{a = a} \emph{after both sides compute} to the same normal form.  Definitional unfolding is free: \li{2 + 3 = 5} is \li{rfl} because the kernel can evaluate it.
...
\subsection*{\li{intro} --- peel off hypotheses}

If the goal is \li{p -> q} (or \li{∀ x, P x}), \li{intro hp} moves the antecedent into the context as hypothesis \li{hp}, leaving goal \li{q}.  It is the tactic form of writing \li{fun hp => \ldots}.

\subsection*{\li{exact} and \li{apply} --- supply the proof}

\li{exact h} closes the goal with the term \li{h}, which must match exactly.  \li{apply h} works \emph{backwards}: if \li{h : p -> q} and the goal is \li{q}, then \li{apply h} replaces the goal by \li{p}.  (See the \li{intro}/\li{apply}/\li{exact} example above --- the term-style proof \li{h hp} is what the three tactics assemble.)

\subsection*{\li{rw} --- rewrite with an equation}

\li{rw [h]}, for \li{h : a = b}, replaces occurrences of \li{a} in the goal by \li{b} (use \li{rw [← h]} for the other direction, and \li{rw [h] at h2} to rewrite inside a hypothesis).
...
\subsection*{\li{simp} --- the simplifier}

Rewrites with every \li{@[simp]} rule (plus any extras you pass in brackets) until nothing changes, then tries to close the goal.  \li{simp [foo]} adds \li{foo} to the rule set; \li{simp at h} simplifies a hypothesis; \li{simpa using h} simplifies both the goal and \li{h}, then closes the goal with it.  \li{simp only [\ldots]} uses \emph{only} the listed rules --- more predictable, common in this book.
...
\subsection*{\li{induction \ldots with} --- structural induction}\label{sec:primer-induction}

\li{induction n with} splits the goal into one case per constructor, giving the step case an induction hypothesis.  Under the hood there is no primitive ``induction'': every \li{inductive} type \li{T} comes with a \emph{recursor} \li{T.rec}, a function stating exactly the induction principle, and the tactic merely applies it.  Seeing this once is worth a hundred words:
...
The \li{motive} is the property being proved, as a function of \li{n} --- what a paper proof calls ``$P(n)$''.  Chapter~1's \li{mynat\_ind} is precisely \li{MyNat}'s recursor restricted to \li{Prop}.

\subsection*{\li{cases} --- case analysis without induction}

Like \li{induction} but with no induction hypothesis: just split on the constructors.  Works on data (\li{cases k with | zero => \ldots | succ k => \ldots}) and on logical connectives, since \li{Or}, \li{And}, \li{Exists} are themselves inductive types:
...
\subsection*{\li{obtain} / \li{rcases} --- destructure in one step}

\li{obtain ⟨n, hn⟩ := h} takes \li{h : ∃ n, P n} apart into a witness \li{n} and a proof \li{hn}; \li{rcases} is the same with nested patterns (\li{⟨n, hgt, hlt⟩} flattens a $\land$ inside an $\exists$) and alternation (\li{h1 | h2} for $\lor$).  The desugaring is a \li{match} --- or, fully explicitly, the eliminator \li{Exists.elim}:
...
(Why can a \li{match} take an $\exists$ apart?  Because \li{Exists} is an ordinary inductive type with one constructor \li{Exists.intro}, so matching on it is just \li{cases}.)

\subsection*{\li{constructor} --- build the goal's constructor}

If the goal's head is an inductive with one applicable constructor (\li{∧}, \li{↔}, a structure), \li{constructor} applies it, turning e.g.\ a goal \li{p ∧ q} into two goals \li{p} and \li{q}.  The \li{·} bullets then focus one subgoal each.
...
\subsection*{\li{calc} --- chained (in)equalities}

A \li{calc} block strings together steps \li{a ≤ b := proof}, each line picking up where the previous ended.  Mixed relations are allowed when transitivity makes sense ($=$ with $\leq$, $\leq$ with $<$, \ldots).  The desugaring: each adjacent pair is glued by the \li{Trans} type class's \li{Trans.trans}:
...
\subsection*{\li{have}, \li{show}, and \li{suffices} --- structuring proofs}

\li{have h : P := proof} adds an intermediate fact to the context.  \li{show P} re-states the current goal (useful to force an unfolding, or just for readability).  \li{suffices h : P by tac} argues top-down: ``it suffices to prove \li{P}, because \li{tac} derives the goal from it''; the remaining goal is then \li{P}.
...
\subsection*{\li{omega} --- linear integer arithmetic, decided}

A decision procedure for linear arithmetic over \li{Nat} and \li{Int} (with \li{+}, constants multiples, \li{≤}, \li{<}, \li{=}, \li{min}/\li{max}, \li{\%} by constants).  If the goal follows from the linear facts in context, \li{omega} closes it --- no hints needed.  It knows nothing about multiplication of variables or about \li{Real}.
...
\subsection*{\li{decide} --- run the decision procedure}

For goals whose truth is \emph{decidable by computation} (a \li{Decidable} instance exists), \li{decide} simply evaluates it and checks the answer is \li{true}.  Works for concrete arithmetic and for quantifiers over finite types; useless for infinite quantifiers or \li{Real}.
...
\subsection*{\li{grind} --- core Lean's general closer}

A core-Lean automation tactic combining congruence closure with arithmetic; in Part~I we use it for routine \li{Rat} algebra (where \li{omega} does not apply and Mathlib's \li{linarith} is not yet available).
...
\subsection*{\li{funext} --- functions equal pointwise are equal}

Not a tactic but a lemma (usable with \li{apply}): to prove \li{f = g} for functions, prove \li{∀ x, f x = g x}.  Chapter~5's \li{addReal} uses it to compare two lifted functions.
...
\section{Quotients: \li{Quot}, \li{Quot.mk}, \li{Quot.lift}, \li{Quot.sound}}

Given a relation \li{r : α → α → Prop}, the type \li{Quot r} is ``$\alpha$ with \li{r}-related elements glued together.''  Four primitives run the whole show:

\begin{itemize}[noitemsep]
  \item \li{Quot.mk r a} --- the class of \li{a} (write $[a]$ on paper);
  \item \li{Quot.sound : r a b → Quot.mk r a = Quot.mk r b} --- related representatives give \emph{equal} classes;
  \item \li{Quot.lift f h} --- descend \li{f : α → β} to \li{Quot r → β}, \emph{provided} the proof \li{h} that \li{f} respects \li{r} (well-definedness!);
  \item \li{Quot.ind} --- to prove something for every class, prove it for every \li{Quot.mk r a}.
\end{itemize}

A complete miniature (integers-mod-2 as a quotient of \li{Nat}):
...
This is exactly the machinery behind \li{MyInt} (Chapter~2) and \li{MyReal} (Chapter~5); there, the ``respect'' proofs are the well-definedness arguments, and binary operations need a nested \li{Quot.lift} with one respect proof per argument.

\section{Mathlib Tactics (Parts II--VII)}

All entries below require \li{import Mathlib}.

\subsection*{\li{linarith} --- linear arithmetic over ordered fields \textbf{(Mathlib)}}

The workhorse of our $\varepsilon$--$\delta$ proofs: closes any goal that is a linear consequence of linear hypotheses over \li{Real} (or \li{Rat}, \li{Int}, \ldots).  Extra facts can be passed inline: \li{linarith [abs\_nonneg x]}.
...
\subsection*{\li{nlinarith} --- a nonlinear boost \textbf{(Mathlib)}}

\li{linarith} after multiplying hypotheses together and adding squares; the bracket hints tell it which products to consider.  Heuristic, not complete --- when it fails, feed it the key square.
...
\subsection*{\li{norm\_num} --- numeric normalisation \textbf{(Mathlib)}}

Evaluates concrete numeric expressions in any ordered field or ring --- like \li{decide}, but for types such as \li{Real} where nothing literally computes, plus extensions (primality, \ldots).
...
\subsection*{\li{positivity} --- positivity by structure \textbf{(Mathlib)}}

Proves goals \li{0 < e}, \li{0 ≤ e}, or \li{e ≠ 0} by recursing over the syntax of \li{e} (products of positives are positive, \ldots).  Ubiquitous for our ``$\delta > 0$'' side-goals.
...
\subsection*{\li{push\_neg} --- normalise negations \textbf{(Mathlib)}}

Pushes \li{¬} inward through quantifiers and connectives: $\lnot\forall$ becomes $\exists\lnot$, $\lnot(a \leq b)$ becomes $b < a$, and so on --- exactly the ``negate the definition of convergence'' step of analysis.  (Recent Mathlib versions are renaming this to \li{push Not}; \li{push\_neg} still works, possibly with a deprecation notice.)
...
\subsection*{\li{by\_contra} --- proof by contradiction \textbf{(Mathlib)}}

\li{by\_contra hx} assumes the negation of the goal as \li{hx} and asks you to derive \li{False}.  It usually travels with \li{push\_neg at hx}.  The desugaring is the classical axiom-powered lemma \li{Classical.byContradiction : (¬p → False) → p}:
...
\subsection*{\li{ring} and \li{ring\_nf} --- commutative-ring identities \textbf{(Mathlib)}}

\li{ring} proves any identity valid in every commutative ring by normalising both sides to a canonical polynomial form; \li{ring\_nf} performs the same normalisation without requiring it to close the goal (we use it to tidy expressions inside $|\cdot|$ before applying the triangle inequality).
...
\subsection*{\li{set} --- name a subexpression \textbf{(Mathlib)}}

\li{set s := e with hs} abbreviates a recurring expression: it replaces every occurrence of \li{e} by a new local \li{s} and records \li{hs : s = e}.  Purely organisational, but it keeps $\varepsilon$--$\delta$ proofs readable.
...
\subsection*{\li{field\_simp} --- clear denominators \textbf{(Mathlib)}}

Multiplies out fractions, using nonzeroness hypotheses from the context, typically leaving a polynomial goal for \li{ring} or \li{linarith}.
...
\subsection*{\li{by\_cases} and \li{unfold}}

Two small players seen in Chapter~36: \li{by\_cases h : p} splits the proof into the case where \li{p} holds and the case where it fails (classically, any proposition may be split; for decidable \li{p} it is constructive).  \li{unfold foo} replaces \li{foo} by its definition in the goal --- a blunter, more predictable cousin of \li{simp only [foo]}.

\bigskip
\noindent That is the complete toolbox.  If a proof in the book uses something not listed here, it is a named lemma, not a tactic --- and Lean's \li{exact?}, or a search on \url{https://leanprover-community.github.io/mathlib4_docs}, will find it for you.
