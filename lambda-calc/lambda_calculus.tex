...
% ── Encoding & Fonts ─────────────────────────────────────────────
\usepackage{fontspec}
% Pin the full system DejaVu by path: TeX-trimmed DejaVu can silently drop
% some math glyphs used in the Lean listings. The system TTFs carry them.
...
That is enough to start reading. As the code grows we lean on a handful of
Lean conveniences---\texttt{do}/\texttt{←} notation, \texttt{<|>},
\texttt{guard}, \texttt{Id.run do} with \texttt{let mut}, \texttt{deriving},
\texttt{partial}/\texttt{termination\_by}, and the odd \texttt{by omega}. Each
is introduced in passing, but if any feels like magic, Appendix~\ref{app:primer}
(``A Lean\,4 Primer'') collects them in one place and shows the plain
\texttt{match}/recursor code each one desugars to.
...
We have the theory of syntax in hand. Over the rest of this book we will turn
it into running Lean code, building up step by step to De Bruijn indices, type
checking, and ultimately a dependent type checker that verifies proofs. The
first step is the humblest one: choosing a data structure for a $\lam$-term.
...
\begin{warning}[Why \texttt{partial} here?]
Both functions are marked \texttt{partial}. Lean normally insists that every
\texttt{def} terminate, and it proves this automatically only when each
recursive call is on a \emph{structural subterm} of an argument. Neither
function qualifies: \texttt{freshVar.go} recurses on \texttt{n + 1} (which
\emph{grows}), and \texttt{subst}'s Rule~5 recurses on
\texttt{subst y (.var z) e}---a freshly \emph{built} term, not a piece of the
input. Without \texttt{partial}, Lean rejects both with ``fail to show
termination.'' Marking them \texttt{partial} tells Lean ``trust me, this
halts'' and opts out of the termination check. (Both really do halt: names
eventually stop colliding, and the rename recursion shrinks the term's
$\lambda$-nesting even though Lean can't see it.) The alternative---a
\texttt{termination\_by} clause that spells out a decreasing measure---is
shown in Appendix~\ref{app:primer}.
\end{warning}
...
Because \texttt{Term} derives \texttt{Repr}, \texttt{\#eval} prints the raw
constructor tree (\texttt{Term.lam "y" (Term.var "y")}), not the pretty
$\lam$-notation. Once we add a \texttt{ToString} instance
(Appendix~\ref{app:code}), \texttt{IO.println (toString (eval 100 t))} prints
the friendly form \texttt{(λy. y)} instead. Either way the value is the same:
$(\lam y.\, y)$ and $a$, exactly the reductions we expected.
...
\begin{theorem}[Normalization / Leftmost Reduction]
If a term has a normal form, normal-order (leftmost-outermost) reduction will
find it. Applicative order may diverge even when a normal form exists.
\end{theorem}

(This is the \emph{normalization theorem}, sometimes called the leftmost- or
standard-reduction theorem. Do not confuse it with the separate
\emph{Standardization Theorem}, which says any reduction sequence can be
reordered into a standard one.) The normalization theorem was proved by Curry
and Feys in 1958, and the operational picture was sharpened
...
\begin{intuition}[Normal form vs.\ diverges-when-applied]
It is easy to blur two different questions: ``does reducing this term
terminate?'' and ``does this term loop?'' Keep them apart.

A term \textbf{has a normal form} if repeatedly firing redexes (in normal
order) eventually reaches a term with \emph{no} redex left. That is a property
of the term \emph{as written}. For example, $\lam x.\, (x\,x)\,(x\,x)$ is
\emph{already} a normal form: its only applications are $x\,x$, and $x$ is a
plain variable, not a $\lam$, so there is nothing to reduce. Normalizing it
halts immediately.

A term \textbf{diverges when applied} if \emph{supplying it an argument}
creates an unbounded reduction. The very same $\lam x.\, (x\,x)\,(x\,x)$
loops forever the moment you apply it to a self-duplicator like
$\lam y.\, y\,y$---that rebuilds $\Omega$. But that divergence lives in the
\emph{application}, not in the original (already-normal) function.

So a term can sit in a perfectly good normal form and still build a machine
that loops once you feed it input---exactly like a Lean function that is
well-defined but would run forever on certain arguments. When an exercise asks
you to ``normalize the body,'' answer the first question; do not assume a
self-application must diverge.
\end{intuition}
...
def K    : Term := .lam "x" (.lam "y" (.var "x"))   -- named K = λx.λy. x
def K_db : Expr := .lam (.lam (.bvar 1))            -- de Bruijn K = λ.λ. 1
...
--  with "type of theorem modus_ponens is not a proposition". The ∧/∨/∃
...
def DTerm.subst (k : Nat) (s : DTerm) : DTerm → DTerm
  ...
  | .app e₁ e₂ => .app (DTerm.subst k s e₁) (DTerm.subst k s e₂)
...
-- position first: in `((λ.λ.·) X) Y` the head `(λ.λ.·) X` is itself a redex,
...
partial def dTypeCheck (ctx : List DTerm) : DTerm → Option DTerm
  ...
  | .app e₁ e₂ => do
    ...
    | .pi dom cod =>
      ...
      some (DTerm.subst 0 e₂ cod)            -- CRUCIAL: substitute the actual
...
\texttt{DTerm.subst 0 e₂ cod} (substitute the argument \texttt{e₂} for
index~0 in the codomain \texttt{cod}). This is where dependent types earn their name. In
...
\begin{warning}[Always bounds-check the position]
Both combinators test \texttt{pos < s.length} \emph{before} reading. This is
not optional. In current Lean, \texttt{s.get ⟨pos⟩} past the end of the string
returns the \emph{default} character \texttt{'A'} rather than failing---and
\texttt{'A'.isAlphanum} is \texttt{true}, so an unguarded
\texttt{many (satisfy Char.isAlphanum)} would ``succeed'' forever and loop
until the stack blows (``deep recursion''). The recursive-descent parser guards
its reads the same way; combinators must too.
\end{warning}
...
\begin{warning}[Reading order vs.\ compilation order]
The four functions above (\texttt{pure}, \texttt{fail}, \texttt{bind},
\texttt{orElse}) are shown as \emph{standalone} definitions to demystify what
the operators do. In the finished parser we \emph{drop} these standalone
\texttt{def}s and instead register the \texttt{Functor}/\texttt{Applicative}/%
\texttt{Monad}/\texttt{Alternative} instances (a few subsections down)---those
instances are what make \texttt{>>=}, \texttt{do}-notation, and \texttt{<|>}
available. Lean resolves those operators only \emph{after} the instances are in
scope, so in an actual source file the instance block must come \emph{before}
the combinators that use them (\texttt{many1}, \texttt{spaces}, \texttt{token},
\texttt{ident}, \texttt{lamP}, \ldots). We present them in conceptual order
here for readability; the compile-ready ordering is: instances first, then the
repetition/whitespace helpers, then the grammar (in a \texttt{mutual} block).
\end{warning}
...
\begin{warning}[These two listings are illustrative sketches]
The \texttt{pipeline} and \texttt{repl} listings above are written for
readability, using suggestive names---\texttt{Defs}, \texttt{display},
\texttt{printDefs}, \texttt{parseLetBinding}, \texttt{defs.insert}---that we
have \emph{not} defined. They stand in for concrete pieces that the runnable
implementation spells out differently. In the complete, compile-checked code
of Appendix~\ref{app:code}: \texttt{Defs} is simply
\texttt{List (String × Term)}; ``display'' is the \texttt{ToString} instance
(so \texttt{s!"\{result\}"}); ``printDefs'' is a \texttt{for} loop over the
list; ``parseLetBinding'' is \texttt{rest.splitOn " = "}; and
``\texttt{defs.insert name term}'' is just consing \texttt{(name, term) ::
defs}. Read this section for the \emph{shape} of the pipeline; read the
appendix for code you can paste and run.
\end{warning}
...
That last result---$(\lam.\, \lam.\, 1\;(1\;0))$---is the Church numeral
$\church{2}$: applying successor twice to zero gives 2. (Recall that the
numeral $\church{n}$ is $\lam f.\, \lam x.\, f^n\,x$; in de Bruijn form the
bound $f$ is index~1 and $x$ is index~0, so the applied head is~1 and the
innermost argument is~0.)
...
\begin{warning}[Schematic sketch --- not runnable as written]
This \texttt{typedPipeline} is deliberately schematic. As written it would
\emph{not} type-check: \texttt{toDeBruijn} produces the \emph{untyped}
\texttt{Expr}, but \texttt{typeCheck} expects the \emph{annotated}
\texttt{TExpr} (a lambda carries its parameter type). A real typed pipeline
needs an annotated front end all the way through---a parser that reads
$\lam(x{:}T).\,e$, a named-to-index conversion for the annotated AST, and only
then \texttt{typeCheck}. The next subsections build exactly that annotated
grammar; the point here is the \emph{shape} of the extended pipeline, not a
drop-in definition.
\end{warning}
...
%======================================================================
\appendix
\section{A Lean\,4 Primer}\label{app:primer}
%======================================================================

This book uses a handful of Lean\,4 conveniences that a reader coming from
mainstream programming may not have met, and a couple of proof tactics. None of
them are essential to the \emph{ideas}---they are ergonomic sugar over plain
recursion and pattern matching. This appendix introduces each one and, where it
matters, shows the ``desugared'' code it expands to. Everything here compiles on
\texttt{leanprover/lean4:v4.31.0} (core, no imports).

\subsection*{\texttt{deriving}: generated instances}

When you write \texttt{deriving Repr, DecidableEq, Inhabited} after an inductive
type, Lean \emph{synthesizes} the corresponding instances for you, so you don't
hand-write them:
...
\texttt{Repr} gives a printable form (what \texttt{\#eval} shows);
\texttt{DecidableEq} gives decidable equality (and hence \texttt{==});
\texttt{Inhabited} names a default element (used by ``partial'' fallbacks like
\texttt{List.head!}). You could write all three by hand; \texttt{deriving} just
generates the obvious code.

\subsection*{\texttt{structure} and \texttt{instance}: type classes}

A \texttt{structure} is a record; an \texttt{instance} registers an
implementation of a type class (an interface). Once registered, operators like
\texttt{+}, \texttt{==}, \texttt{>>=}, \texttt{<|>} resolve to it automatically:
...
This is exactly how the parser-combinator section makes \texttt{Parser} a
\texttt{Monad} and \texttt{Alternative}: register the instances, then use
\texttt{do} and \texttt{<|>}.

\subsection*{\texttt{do} / \texttt{←}: the \texttt{Option} monad}

The single most-used piece of sugar in this book. Inside a \texttt{do} block for
\texttt{Option}, \verb|let a ← e| means ``if \texttt{e} is \texttt{some a},
continue; if it is \texttt{none}, abandon the whole block and return
\texttt{none}.'' It removes a staircase of nested \texttt{match}es. All three of
these are the \emph{same} function:
...
\verb|←| is \texttt{bind} plus a name; \texttt{pure x} is \texttt{some x};
\texttt{bind} is ``match, propagate \texttt{none}.'' Whenever you see \texttt{do}
in \texttt{toDeBruijn}, \texttt{typeCheck}, or the parser, mentally expand it to
form (3).

\subsection*{\texttt{guard}: a mid-block failure test}

\texttt{guard p} succeeds (with \texttt{()}) when \texttt{p} holds and fails
(\texttt{none}) otherwise. In a \texttt{do} block it is a guard clause:
...
The dependent type checker uses \texttt{guard (dom.beq argTy)} the same way:
``unless the argument type matches, reject.''

\subsection*{\texttt{<|>}: choice (\texttt{Alternative})}

\verb|p <|> q| means ``try \texttt{p}; if it fails, try \texttt{q}.'' For
\texttt{Option} it is just:
...
The parser combinators use \verb|lamP <|> appP| to mean ``a lambda, or else an
application.''

\subsection*{\texttt{\_ matches \_}: a pattern as a Bool}

\verb|e matches pat| is \texttt{true} exactly when \texttt{e} fits the pattern.
It desugars to a \texttt{match} that returns a \texttt{Bool}:
...
The checker writes \texttt{aTy.whnf matches .univ} to ask ``did this reduce to
\texttt{Type}?''

\subsection*{\texttt{Id.run do} + \texttt{let mut}: imperative-looking folds}

Lean has no mutable state, but \texttt{Id.run do} lets you \emph{write} code that
looks imperative---\texttt{let mut} plus reassignment---and compiles it down to a
pure fold. These two are equal:
...
The Church-encoding definition lists (\texttt{Id.run do} \ldots\ \texttt{let mut
ds := \ldots}) use this to build a list step by step; it is a fold in disguise.

\subsection*{\texttt{partial} and \texttt{termination\_by}}

Lean accepts a recursive \texttt{def} only once it is convinced the recursion
\emph{stops}. It does this automatically when each recursive call is on a
structural piece of an argument. When it cannot see why a function halts, you
have two choices.

\textbf{Opt out with \texttt{partial}.} This tells Lean ``trust me, this
terminates'' and skips the check. It is what \texttt{freshVar}, \texttt{subst}
(named version), \texttt{whnf}, and the parser use:
...
\textbf{Prove it with \texttt{termination\_by}.} When there \emph{is} a measure
that shrinks, name it, and discharge the ``it decreases'' goal (often
\texttt{omega} suffices):
...
The \texttt{fuel} parameter used for \texttt{eval} is a third option: make the
recursion structural on a \texttt{Nat} that always counts down, so no proof is
needed at all.

\subsection*{\texttt{induction} / \texttt{cases} $\leftrightarrow$ the recursor}

\texttt{induction} and \texttt{cases} are tactics, but they are not primitive:
each compiles to a call of the type's automatically generated \emph{recursor}
(\texttt{T.rec}). Here is a proof both ways---first with the tactic, then as the
raw term it elaborates to:
...
\texttt{cases} is the same idea with no induction hypothesis---it just splits on
the constructor:
...
\subsection*{\texttt{obtain ⟨…⟩} $\leftrightarrow$ \texttt{Exists.elim} / \texttt{match}}

\texttt{obtain} destructures a hypothesis. For an existential it pulls out the
witness and its property; underneath, that is \texttt{Exists.elim} (equivalently
a \texttt{match}):
...
The anonymous-constructor brackets \verb|⟨n, hn⟩| build \emph{and} take apart
structures (pairs, \texttt{And}, \texttt{Exists}, \texttt{Point}, \ldots).

\subsection*{\texttt{by omega}: linear arithmetic, decided}

\texttt{omega} is a decision procedure for linear arithmetic over \texttt{Nat}
and \texttt{Int} (equalities, \verb|<|, \verb|≤|, \texttt{+}, constant multiples,
and their Boolean combinations). It is \emph{not} magic and \emph{not} a general
prover---it just happens to settle the small arithmetic goals we hit:
...
When you see \texttt{by omega} in the book (e.g.\ the BHK \texttt{exists\_gt\_5}
example), read it as ``the arithmetic here is routine---\texttt{omega} checks
it.''
...
Normalizing the body (normal order) actually \emph{terminates} in three more steps:
\[
  \lam x.\, \omega\,(\omega\,x)
  \reduce \lam x.\, (\omega\,x)\,(\omega\,x)
  \reduce \lam x.\, (x\,x)\,(\omega\,x)
  \reduce \lam x.\, (x\,x)\,(x\,x).
\]
The result $\lam x.\, (x\,x)\,(x\,x)$ \textbf{is a normal form}: its only
applications are $x\,x$, and $x$ is a variable, not a $\lam$, so there is no
redex to fire. The term does \emph{not} diverge on its own. Divergence appears
only if you later \emph{apply} this normal form to an argument---feeding it,
say, $\omega$ or itself re-creates $\Omega$-style self-application that loops.
Having a normal form and looping-when-applied are different properties; see the
box ``Normal form vs.\ diverges-when-applied'' in Section~\ref{sec:strategies}.
...
\medskip
\textbf{6.5.}\; We need $(\lam y.\, x\;y)[x := \lam y.\, y]$. The binder is $\lam y$, so we check whether the replacement term $\lam y.\, y$ has $y$ free. It does not: $\FV(\lam y.\, y) = \varnothing$ (the $y$ inside $\lam y.\, y$ is bound by its own $\lam$). Since $y \notin \FV(\lam y.\, y)$, no capture is possible, so \textbf{rule 4 (the safe case) applies}---push the substitution under the binder unchanged: $\lam y.\, (x\;y)[x := \lam y.\, y] = \lam y.\, (\lam y.\, y)\;y$. The result $\lam y.\, (\lam y.\, y)\;y$ reduces to $\lam y.\, y$, which is correct ($\eta$-equivalent to the original).
...
\textbf{11.1.}\; The proof tree for the $\mathbf{S}$ combinator has type $(A \to B \to C) \to (A \to B) \to A \to C$. It requires three \textsc{Abs} rules (one for each $\lam$), three \textsc{App} rules (one for $x\;z$, one for $y\;z$, and one combining them into $x\;z\;(y\;z)$), and three \textsc{Var} axioms ($x$, $y$, $z$):
...
\textbf{10.1.}\; The na\"ive version fails on $\lam.\,\lam.\, 0\;1\;2$:
...
\medskip
\textbf{10.2.}\; $\texttt{substV2}\;0\;(\texttt{bvar}\;0)\;(\lam.\, 1)$:
...
\medskip
\textbf{10.3.}\; (a) Replace with $s$ when $n = k$; leave as $\texttt{bvar}\;n$
when $n \neq k$.
...
\medskip
\textbf{10.4.}\; $k$ goes $0 \to 1 \to 2 \to 3$. Yes, index $3$ inside three binders refers to the same variable as index $0$ outside. The substitution fires correctly.

\medskip
\textbf{10.5.}\; See the worked trace in the exercise box.

\medskip
\textbf{10.6.}\; Body is $\lam.\, 1$, argument is $\texttt{bvar}\;0$.
