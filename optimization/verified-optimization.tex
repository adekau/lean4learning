...
% Pin the FULL system DejaVu by path: the TeX-trimmed DejaVu shipped with
% TinyTeX drops some math glyphs used in the Lean listings (e.g. the join
% symbol U+2294 ⊔). The system TTF has them.
...
\newcommand{\li}[1]{{\ttfamily\small\color{deepblue}#1}}
...
World vs.\ World game mode.  You choose stat combinations for fourteen equipment
...
gear solver in Lean\,4, developed alongside a \emph{specification} of what it
means for that solver to be correct---and, where the proofs are short enough,
machine-checked verification of the pieces.

\begin{warningbox}[title={An honest word about ``verified''}]
This is a book about \emph{how to build and specify} a verified solver, not a
finished verified artifact.  Several of the deepest correctness
theorems---solver soundness, branch-and-bound invariant preservation, LP-bound
validity, infusion-DP optimality---are stated precisely as \emph{specifications}
and their proofs are left as extended exercises (shown in Lean with the
\li{sorry} placeholder, which means ``unproven''---see
Appendix~\ref{app:primer}).  We are careful never to present a \li{sorry}-backed
theorem as though it were machine-checked.  What \emph{is} genuinely proved
(irreflexivity and transitivity of Pareto dominance, decidability of dominance,
the small worked lemmas) is called out as such.  Treat the capstone soundness
theorem as a rigorously-stated contract with a proof sketch, in the spirit of a
specification you would hand to a verification team---not as a closed proof.
\end{warningbox}

This is a book about \textbf{discrete (combinatorial)} optimisation: the gear
problem picks one combo per slot from a finite menu, so there are no gradients
to descend.  We survey the continuous world (convexity, linear programming) only
where it feeds the discrete solver---LP relaxation as a bounding technique.
Gradient descent and Newton's method are named for contrast and then set aside;
they are not developed here.  The mathematics we \emph{do} develop is real:
linear programming and duality, integer programming, branch and bound,
constraint propagation, Lagrangian relaxation, Pareto optimality, and dynamic
programming---each motivated by the gear problem, implemented in Lean\,4, and
connected back to the lattice-theoretic foundations you already know.

\subsection*{Prerequisites and Lean setup}

Comfort with reading Lean\,4 (typeclasses, basic tactic proofs) and the material
from \emph{Verified Order Theory}---especially partial orders, lattices,
join-semilattices, propagator cells, and the Knaster--Tarski fixed-point
theorem.  No deep proof-assistant experience is required:
Appendix~\ref{app:primer}, \emph{A Lean 4 Primer}, explains every construct and
tactic this book uses (\li{intro}, \li{exact}, \li{constructor}, \li{obtain},
\li{unfold}, \li{rfl}, \li{simp}, \li{omega}, \li{split}, and an honest account
of \li{sorry}), each with a compiled example.

\medskip\noindent\textbf{Toolchain and libraries.}  All Lean code targets
toolchain \li{leanprover/lean4:v4.31.0}.  The book mixes two kinds of listing.
Most of the core data model, the heuristics, the infusion DP, and the Pareto
proofs are \textbf{plain core Lean} (no external packages).  But the propagators,
the whole solver loop, and the A* skeleton depend on \textbf{Mathlib}
(\li{Finset}, \li{SemilatticeSup}/\li{⊔}) and \textbf{Batteries}
(\li{BinaryHeap}).  Section~\ref{sec:mathlib-setup} states this dependency
explicitly and shows how to set up a Mathlib project.  Every listing is
machine-checked: the core ones against a bare toolchain
(\li{OptimizationVerification.lean}), the Mathlib ones against Mathlib
v4.31.0 (\li{OptimizationMathlibVerification.lean}).  Where a listing
deliberately leaves a \li{sorry}, the surrounding text says so.
...
Before defining a build, we need three enumerations---the list of all gear
slots, the list of all stat types, and a menu of stat combos---plus a
placeholder \li{Rune} type.  These are referenced throughout the book, so we
give them once here (a fuller combo menu appears in Appendix~\ref{app:combos}):

\begin{leanbox}[title={Lean 4 --- Enumerations and Runes}]
\begin{minted}{lean}
...
There are 14 gear slots, so \li{GearSlot.all} has 14 entries; the search-space
and termination bounds later in the book count from this list.
...
\begin{warningbox}[title={Soft objective vs.\ hard floors: a modelling decision}]
The penalty above is a \emph{soft} objective: missing a target is penalised
(heavily), not forbidden.  But the propagators and solver we build from
Chapter~\ref{ch:stronger} onward treat each stat target as a \textbf{hard
floor}: the \texttt{LinearGeq} propagator \emph{deletes} any combo that cannot
reach a target.  These are two \emph{different} problems, and it matters which
one we are solving:
\begin{itemize}[nosep]
  \item \textbf{Soft-penalty problem:} minimise $\pen(b)$ over \emph{all} builds.
    Here a small shortfall on one stat may be the best outcome.
  \item \textbf{Hard-floor problem (what the solver decides):} among builds that
    meet \emph{every} target ($\statTotal(b,s) \geq T(s)$ for all $s$), minimise
    the (overshoot) penalty; report infeasible if none exist.
\end{itemize}
The solver in Part~IV solves the \emph{hard-floor} problem, and its soundness
theorem (Chapter~\ref{ch:correctness}) is stated against a feasibility predicate
that includes the floors.  This is a legitimate and common choice (targets as
requirements), but it is \textbf{not} the same as minimising the soft penalty:
\texttt{LinearGeq} pruning can discard the soft-penalty optimum (a combo giving
the \emph{smallest} shortfall---hence lowest penalty---may still fail to reach
the floor and be pruned).  We flag this again where the propagator is defined,
and the correctness theorem is stated precisely to match the code.
\end{warningbox}

\subsection{The Search Space}

With 40 stat combos and the 14 gear slots of our \li{GearSlot} type, the raw
search space is $40^{14} \approx 6.9 \times 10^{22}$ gear assignments---before
...
\section{Which Parts Need Mathlib}\label{sec:mathlib-setup}

The next listing is the first in the book to reach outside core Lean.  A
\li{Cell} is parametrised by a \li{SemilatticeSup} (a join-semilattice), and a
gear-slot domain is a \li{Finset StatCombo}---both from \textbf{Mathlib}, the
Lean community's mathematics library.  Later, the A* skeleton of
Chapter~\ref{ch:search} uses \li{BinaryHeap} from \textbf{Batteries}.  Core Lean
knows none of these.  So the book's listings split into two groups:

\begin{itemize}[nosep]
  \item \textbf{Core Lean} (no packages): the data model, the penalty, the
    heuristics (hill climbing, simulated annealing, tabu, GA), the infusion DP,
    the Pareto dominance proofs, and the \li{BitVec} performance code.  These are
    machine-checked in \li{OptimizationVerification.lean}.
  \item \textbf{Mathlib/Batteries}: everything that mentions \li{Finset},
    \li{SemilatticeSup}/\li{⊔}, or \li{BinaryHeap}---the propagators, the
    backtracking/branch-and-bound solver, and A*.  These are machine-checked in
    \li{OptimizationMathlibVerification.lean}.
\end{itemize}

\begin{intuitionbox}[title={Setting up a Mathlib project}]
Create a project directory with a \li{lean-toolchain} file containing the single
line \li{leanprover/lean4:v4.31.0}, and a \li{lakefile.toml}:
...
Then run \li{lake update} to resolve the dependency and \li{lake exe cache get}
to download prebuilt \li{.olean} binaries---do not skip this, or your machine
will spend hours compiling Mathlib from source.  All fetched packages and build
artifacts live under the project's \li{.lake/} directory.  Begin the
Mathlib-dependent files with \li{import Mathlib} (Batteries comes with it; the
A* code additionally does \li{open Batteries}).  The core-Lean listings need no
project at all---\li{lake env lean File.lean} against the bare toolchain
suffices.
\end{intuitionbox}

\begin{leanbox}[title={Lean 4 (Mathlib) --- Propagator Network (from the Lattice Book)}]
\begin{minted}{lean}
...
slot instead of 40.  That's still $15^{14} \approx 2.9 \times 10^{16}$
possible builds.
...
the objective is the asymmetric penalty.  Concretely, a \emph{gear problem}
bundles the stat targets, the per-slot initial domains, and (for the
scalarisation sweep of Chapter~\ref{ch:extensions}) an optional per-stat
miss-weight:

\begin{leanbox}[title={Lean 4 (Mathlib) --- GearProblem}]
\begin{minted}{lean}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- LinearGeq Propagator}]
\begin{minted}{lean}
...
\begin{warningbox}[title={LinearGeq is sound for hard floors, not for the soft penalty}]
\texttt{LinearGeq} removes $c$ from $D_i$ exactly when combo $c$ cannot reach
$T(s)$ even granting slot $i$ the whole 90-point infusion budget and every other
slot its best combo.  This is a valid \emph{necessary} condition for the
\textbf{hard-floor} problem: any build meeting all floors keeps every combo it
uses (this is the honest restatement \texttt{propagation\_preserves\_feasible}
of Chapter~\ref{ch:correctness}).  It is \emph{not} sound for the pure
soft-penalty objective: the very combo that misses a floor by the smallest
amount may be the soft-penalty minimiser, yet \texttt{LinearGeq} deletes it.
Throughout, the solver commits to the hard-floor reading.
\end{warningbox}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- LinearLeq Propagator}]
\begin{minted}{lean}
...
\begin{propbox}[title={Monotonicity of LinearLeq (nonempty domains)}]
\texttt{LinearLeq} is monotone \emph{as long as no domain is empty}: if the
domains shrink but stay nonempty, the propagator removes at least as many combos.
\end{propbox}

\begin{proof}
When a \emph{nonempty} domain shrinks ($\emptyset \neq D'_j \subseteq D_j$), the
minimum contribution from other slots can only \emph{increase} (the minimum over
a smaller nonempty set is $\geq$ the minimum over a larger one):
$\minContrib'(j, s) \geq \minContrib(j, s)$.
So $L'(i, c, s) \geq L(i, c, s)$, and any combo removed by the old propagator
is still removed.
\end{proof}

\begin{warningbox}
The minimum of an \emph{empty} set has no natural value.  We use the convention
$\minContrib(\emptyset, s) = 0$ (via \li{.getD 0} above), which makes
\li{minContribForStat} total, but it breaks the ``min over a smaller set is
larger'' step: an empty domain reports $0$, the \emph{smallest} possible value.
Since an empty domain already signals infeasibility (handled separately by the
solver), this is harmless in practice; but the monotonicity claim genuinely
needs the nonempty hypothesis stated above.  This is a real gap the earlier
edition glossed over.
\end{warningbox}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Infusion Budget Propagator}]
\begin{minted}{lean}
...
of slot $i$.  With 40 combos and 14 slots, this is at most 560 steps.
\end{thmbox}
...
$40 \times 14 = 560$.  Since the height is a natural number that strictly
...
constraint matrix, and $b \in \R^m$ is the right-hand side.  We use the
\emph{greater-than} form $Ax \geq b$ because the gear problem's stat targets are
\emph{floors} ($\statTotal \geq T$); it is equivalent to the $\leq$ form by
negating rows.
\end{defbox}

\begin{intuitionbox}[title={Transpose and inner-product notation}]
For column vectors $c, x \in \R^n$, the \emph{transpose} $c^\top$ is the row
vector $(c_1,\dots,c_n)$, so $c^\top x = \sum_{i=1}^n c_i x_i$ is the ordinary
\emph{dot (inner) product}---a single number, the weighted total cost.  For a
matrix $A \in \R^{m\times n}$, the transpose $A^\top \in \R^{n\times m}$ swaps
rows and columns, and the identity $(A^\top y)^\top x = y^\top (A x)$ (both equal
$\sum_{i,j} A_{ij}\,x_j\,y_i$) is the one algebraic fact the duality proof below
leans on.  If you have not met this notation, read $c^\top x$ as ``dot product of
$c$ and $x$'' and $A^\top y$ as ``$A$ with rows and columns flipped, times $y$''.
\end{intuitionbox}

The feasible region $\{x \mid Ax \geq b, x \geq 0\}$ is a convex
\emph{polyhedron}---an intersection of finitely many half-spaces.
...
Every LP has a \emph{dual} problem.  For the primal
$\min\; c^\top x$ s.t.\ $Ax \geq b, x \geq 0$,
the dual is $\max\; b^\top y$ s.t.\ $A^\top y \leq c, y \geq 0$.
(One constraint direction flips relative to the other because the dual
lower-bounds a minimisation.)
...
\begin{proof}
$c^\top x \geq (A^\top y)^\top x = y^\top A x \geq y^\top b = b^\top y$.
The first inequality uses dual feasibility ($A^\top y \leq c$ and $x \geq 0$, so
$(A^\top y)^\top x \leq c^\top x$); the second uses primal feasibility
($Ax \geq b$ and $y \geq 0$, so $y^\top A x \geq y^\top b$).  Thus every dual
feasible $y$ gives a \emph{lower} bound $b^\top y$ on the primal minimum
$c^\top x$---exactly what a bounding technique needs.
\end{proof}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Fail-First Variable Selection}]
\begin{minted}{lean}
...
Note the explicit \li{match}: the earlier edition wrote \li{unfixed.head!},
which \emph{panics at runtime} when every domain is already a singleton
(\li{unfixed} empty).  Pattern-matching makes the "all fixed" case total and
lets the caller detect a leaf.
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Value Ordering by Tightest Deficit}]
\begin{minted}{lean}
...
\begin{warningbox}[title={Finsets are for reasoning; lists are for running}]
Mathlib's \li{Finset.toList} is \li{noncomputable} (it picks a representative via
choice), so any solver that enumerates a \li{Finset} domain with \li{.toList}
\emph{cannot execute}.  We keep \li{Finset} in the propagator listings because it
is the right object for the monotonicity proofs, but the executable solver
represents each domain as a \emph{sorted list} of surviving combos (equivalently,
enumerate a \li{Finset} with \li{Finset.sort}).  The one fully runnable
end-to-end component in this book---the infusion DP of
Chapter~\ref{ch:infusion}---is written in core Lean over lists and arrays, and
we \li{\#eval} it on a concrete instance there.
\end{warningbox}
...
\begin{leanbox}[title={Lean 4 --- Symmetry Breaking (specification)}]
\begin{minted}{lean}
...
This theorem is stated as a \emph{specification}: the \li{sorry} means it is
\textbf{unproven} in this book (see Appendix~\ref{app:primer}).  The proof is a
good exercise---the key fact is \li{coefficients .ring1 = coefficients .ring2},
so swapping the two rings leaves every \li{statTotal} and hence the penalty
unchanged.
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Backtracking Search}]
\begin{minted}{lean}
...
Two fixes over the earlier edition: Mathlib's \li{Finset} has no \li{isEmpty}
field (test \li{card = 0}), and \li{Finset.fold} folds a
\emph{commutative--associative} operation, not a general accumulator loop---so
branching enumerates the domain to a list and uses \li{List.foldl}.
...
First, a sanity check that decides everything below.  The largest Power the
three slots can produce \emph{from gear alone} is $141 + 94 + 47 = 282 < 300$.
So the target is unreachable without infusions, and reachable only because the
propagator credits each combo with the full $90$-point infusion budget.  Let us
run the propagator exactly as defined (recall $U(i,c,\text{Power}) =
\texttt{contribution}(i,c,\text{Power}) + \sum_{j\neq i}\maxContrib(j,\text{Power})
+ 90$; keep $c$ iff $U \geq 300$).  These numbers are machine-checked
(\li{ch6Survivors} in \li{OptimizationVerification.lean}).

\paragraph{Step 1a: No infusions ($+0$).}
If we drop the $+90$ term, every combo on every slot fails.  The best any slot
can do is its own max plus the other two slots' maxes: coat sees
$141 + 94 + 47 = 282 < 300$, and every other combo/slot is smaller.  So
\texttt{LinearGeq} empties \emph{every} domain, and the instance is reported
\textbf{infeasible}.  (The earlier edition's claim that propagation leaves
$\{\text{Berserker's}, \text{Marauder}\}$ on each slot is wrong---nothing
survives.)

\paragraph{Step 1b: With the $+90$ infusion budget (as the propagator is
actually defined).}
Now compute $U$ per slot:
\begin{itemize}[nosep]
  \item \textbf{Coat} ($\maxContrib$ of others $= 94 + 47 = 141$):
    Berserker's $141+141+90 = 372$; Marauder $121+141+90 = 352$;
    Celestial $67+141+90 = 298 < 300$.  \emph{Remove Celestial from coat}; keep
    Berserker's and Marauder.
  \item \textbf{Leggings} (others $= 141 + 47 = 188$): Berserker's
    $94+188+90 = 372$; Marauder $81+188+90=359$; Celestial
    $44+188+90 = 322 \geq 300$.  \emph{All three survive.}
  \item \textbf{Boots} (others $= 141 + 94 = 235$): Berserker's
    $47+235+90=372$; Marauder $40+235+90=365$; Celestial
    $22+235+90 = 347 \geq 300$.  \emph{All three survive.}
\end{itemize}
So LinearGeq removes exactly one combo---Celestial on the coat---and the search
space drops from $3^3 = 27$ to $2 \cdot 3 \cdot 3 = 18$.

\paragraph{Step 2: Search.}
Propagation alone did not decide the problem, so we branch.  The coat has the
smallest domain (2), so fail-first picks it.  Fixing the coat and evaluating
leaves the solver to distribute the $90$ infusion points to clear the residual
Power deficit and minimise overshoot on the other stats---which is exactly the
infusion subproblem of Chapter~\ref{ch:infusion}.  For instance,
all-Berserker's gear gives $282$ Power, needing $\lceil (300-282)/5\rceil = 4$
infusions on Power to reach the floor, leaving $14$ for the other targets.

\paragraph{Step 3: Result.}
The instance is feasible \emph{only} once the $90$-point infusion budget is
credited: gear tops out at $282$, and $282 + 90 = 372 \geq 300$.  The solver
returns the floor-meeting build of minimum penalty.  The lesson: the $+90$ term
in \texttt{LinearGeq} is not optional bookkeeping---remove it and this very
instance flips from feasible to infeasible.
...
\begin{leanbox}[title={Lean 4 (Batteries) --- A* Search Skeleton}]
\begin{minted}{lean}
...
(lower) the objective.  Three subtleties the earlier edition got wrong:
\li{SearchNode} cannot \li{deriving Repr} (its \li{domains} field is a function);
Batteries' \li{BinaryHeap} offers only \li{extractMax}, returning a
\emph{pair} (an \li{Option} node and the rest of the heap) rather than an
\li{Option} of a pair; and
because \li{extractMax} pops the \emph{largest} element, the comparator must be
\emph{reversed} (\li{·.fCost > ·.fCost}) so that ``largest under $>$'' is the
smallest $f$-cost that best-first search actually wants.
...
\begin{warningbox}[title={Minimising a nogood soundly is subtle}]
The earlier edition kept only assignments with
\li{contribution slot combo failedStat > 0}.  That is \textbf{unsound} for a
stat-\emph{floor} (LinearGeq) failure: the culprits of a floor miss are exactly
the slots contributing \emph{too little}---often those contributing \emph{zero}
to the failed stat---so dropping them can yield a set that is \emph{not}
inconsistent, wrongly pruning feasible branches.  The safe extractor above keeps
the entire failing assignment.  Shrinking a nogood soundly (retaining only a
minimal inconsistent core) requires re-checking that the retained subset still
forces the floor miss---a QuickXplain-style minimisation, left as an exercise.
\end{warningbox}
...
Five corrections over the earlier edition: there is no \li{List.pairs} in the
library (we define it); in v4.31 \li{Array.zipWith} takes the \emph{function
first}; \li{Float.max} is spelled \li{max}; \li{Array.mkArray} was renamed
\li{Array.replicate}; and the fold's 3-tuple result is unpacked with a
\li{match} (the old \li{|>.map} used \li{Prod.map}, which needs two functions).
Crucially, the step size now uses the \emph{current} index \li{k}, not the fixed
total \li{iters}---the earlier \li{1.0 / (1.0 + iters.toFloat)} was
\textbf{constant} across all iterations, contradicting the ``diminishing''
comment and prose.
...
\section{Reduced-Cost Fixing}\label{sec:reduced}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Reduced-Cost Fixing}]
\begin{minted}{lean}
...
builds reachable by changing one slot.  $|N(b)| \leq 14 \times 39 = 546$.
\end{defbox}
...
giving $|N| \approx 14^2 \times 39^2 \approx 300{,}000$) explores more
...
(In Lean v4.31 \li{List.bind} was renamed \li{List.flatMap}.)
...
Three fixes: \li{Nat} has no maximal element, so the ``worst possible''
sentinel is a large literal, not \li{Nat.max} (which is the binary \li{max}
\emph{function}); \li{local} is a reserved keyword, renamed \li{localBest}; and
the accumulator is a triple, so we project \li{(build, penalty)} explicitly
rather than \li{|>.fst} (which would drop the penalty the signature promises).
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Simulated Annealing}]
\begin{minted}{lean}
...
Two fixes: core Lean has no automatic \li{Nat}$\to$\li{Float} coercion, so the
gap is converted with \li{.toFloat}; and the six-tuple accumulator is unpacked
with a \li{match} (the old \li{|>.map} used \li{Prod.map}, which takes two
functions, not one).
...
A typical value for the gear problem: tenure $\approx \sqrt{14} \approx 4$
(the square root of the number of variables, rounded).
...
This is one of the few results we \textbf{genuinely prove} in this book (no
\li{sorry}): irreflexivity, transitivity, and decidability of dominance all
close.

\begin{leanbox}[title={Lean 4 --- Pareto Dominance and Frontier (fully proved)}]
\begin{minted}{lean}
...
The earlier edition's \li{dominates\_trans} was broken (it wrote
\li{Nat.le\_trans …|>.symm}, but $\leq$ has no \li{.symm}, and a stray tactic
followed \li{exact}), and its \li{Decidable} instance was four \li{sorry}s.  Both
are now genuine proofs; the instance is derived from the boolean test
\li{dominatesBool} via \li{decidable\_of\_iff}, so \li{dominates} not only
type-checks but \emph{runs}.
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- $\varepsilon$-Constraint Sweep}]
\begin{minted}{lean}
...
\begin{leanbox}[title={Lean 4 --- Infusion DP (runnable core Lean)}]
\begin{minted}{lean}
...
This is the one component we run end-to-end.  The earlier edition had seven
distinct bugs here: \li{Array.mkArray} (renamed \li{Array.replicate}); a
three-argument \li{foldl} step (it takes two); a fold body returning a row while
the accumulator was the whole table; \li{.mapIdx} whose index is a \li{Nat}, not
a \li{Fin} with \li{.val}; the \li{Nat.max} sentinel (a function); \li{indexOf}
(renamed \li{idxOf}); and a reversed allocation.  The repaired version above
compiles and executes.  On a two-stat toy instance (no gear, targets
Power~40 and Precision~50, budget~18) it returns the optimum---8 infusions to
Power (=40) and 10 to Precision (=50)---with penalty~0:

\begin{leanbox}[title={Lean 4 --- Running the DP}]
\begin{minted}{lean}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- The Complete Solver}]
\begin{minted}{lean}
...
Several fixes make this both compile and be \emph{sound} for the hard-floor
problem.  \li{simulatedAnnealing} now receives its \li{rng} and its returned
pair is destructured (not treated as a \li{Build}).  The incumbent is an
\li{Option}: a leaf is accepted only after \li{meetsFloors} confirms it meets
every target, so \li{branchAndBound} returns \li{none} exactly when no
floor-meeting build exists---making \li{solve}'s \li{.infeasible} arm
\emph{live} rather than dead code.  Branching enumerates the domain to a list
(\li{Finset.toList} is noncomputable) and folds with \li{List.foldl} (not
\li{Finset.fold}).  \li{solveLPRelaxation} here returns a bare \li{Nat} lower
bound; a richer \li{Float}-valued variant (with reduced costs, for
Section~\ref{sec:reduced} and dual certificates) is a separate interface.
...
This chapter states the solver's correctness \emph{contract} precisely.  We are
honest about status: the top-level theorem and its four component lemmas are
presented as \textbf{specifications}---their Lean statements are exact, but their
proofs are left as extended exercises and appear with \li{sorry} (which means
UNPROVEN; see Appendix~\ref{app:primer}).  We do \emph{not} claim the solver is
machine-checked end-to-end.  What \emph{is} proved elsewhere is called out
(Pareto dominance in Chapter~\ref{ch:pareto}; the runnable infusion DP in
Chapter~\ref{ch:infusion}).

\section{Feasibility and the Main Theorem}

First we pin down \emph{feasibility}.  Because the solver treats stat targets as
hard floors (Chapter~\ref{ch:stronger}), the feasibility predicate must include
those floors---otherwise the \li{.infeasible} arm would be false (every
budget-respecting build trivially ``feasible'') and \li{LinearGeq} pruning could
delete the claimed optimum.

\begin{leanbox}[title={Lean 4 (Mathlib) --- Feasibility (with hard floors)}]
\begin{minted}{lean}
...
\begin{thmbox}[title={Solver Soundness (specification)}]
\begin{minted}{lean}
...
\begin{warningbox}[title={Soft vs.\ hard, restated}]
This theorem does \textbf{not} say the solver minimises the soft penalty of
Chapter~\ref{ch:problem} over \emph{all} builds.  It says: among builds meeting
every hard floor, the solver returns one of minimum penalty, or reports
\li{.infeasible} when none exists.  The soft-penalty minimiser may miss a floor
and hence lie outside this feasible set---\li{LinearGeq} would prune it.  If you
truly want the soft objective, drop the floor conjunct from \li{feasible} and
replace \li{LinearGeq} pruning by penalty-dominance pruning (only delete combos
provably dominated in \emph{penalty}); the theorems below would then need
restating for that model.
\end{warningbox}

\section{Proof Structure}

The proof composes four lemmas.  We present each with its Lean specification
and proof sketch.

\subsection{Propagation Soundness}

\begin{lembox}[title={Propagation Preserves Feasible Solutions}]
If a combo $c$ is removed from slot $i$'s domain by \texttt{LinearGeq}, then no
\emph{floor-feasible} build (one meeting every hard target within the budget)
uses $c$ on slot $i$.  Equivalently: propagation preserves every feasible build.
\end{lembox}

\begin{proof}
\texttt{LinearGeq} removes $c$ from $D_i$ only when, for some stat $s$,
\[
  \texttt{contribution}(i, c, s) + \sum_{j \neq i} \maxContrib(j, s) + 90 < T(s).
\]
Take any build $b$ with $b.\text{gear}(i) = c$ whose other gear also lies in the
domains and whose infusions respect the 18-slot budget (so $5\cdot\text{inf}(s)
\leq 90$).  Then
\[
  \statTotal(b,s) = \texttt{contribution}(i,c,s) + \!\!\sum_{j \neq i}\!\!
  \texttt{contribution}(j, b.\text{gear}(j), s) + 5\cdot\text{inf}(s)
  \;\leq\; \texttt{contribution}(i,c,s) + \!\!\sum_{j\neq i}\!\!\maxContrib(j,s) + 90
  \;<\; T(s).
\]
So $b$ \emph{misses} floor $s$ and is infeasible.  Hence no feasible build uses
$c$ on slot $i$, and the combos of a feasible build all survive.  (Note the
bound uses $+90$ \emph{once}, as a cap on the shared infusion budget---not per
stat.)
\end{proof}

\begin{leanbox}[title={Lean 4 (Mathlib) --- Propagation Soundness (specification)}]
\begin{minted}{lean}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- LP Bound Validity (specification)}]
\begin{minted}{lean}
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- B\&B Invariant (specification)}]
\begin{minted}{lean}
...
\begin{leanbox}[title={Lean 4 --- Infusion DP Optimality (specification)}]
\begin{minted}{lean}
...
\begin{enumerate}[nosep]
  ...
  \item Every other feasible build either was evaluated (and is no better)
    ...
    bound validity).
  \item By propagation soundness (\texttt{propagation\_preserves\_feasible}),
    every pruned combo would force some floor to be missed, so no
    \emph{feasible} build was pruned; the explored space contains all
    feasible builds.
  \item Therefore, the incumbent is a minimum-penalty feasible build.
\end{enumerate}

The infeasibility case: \li{solve} reports \li{.infeasible} either when initial
propagation empties a domain, or when branch-and-bound finishes with no
floor-meeting leaf (incumbent still \li{none}).  By
\texttt{propagation\_preserves\_feasible}, any feasible build would have
survived and been found, so \li{.infeasible} correctly means \emph{no} feasible
build exists.

\begin{leanbox}[title={Lean 4 (Mathlib) --- Composing the Full Proof (specification)}]
\begin{minted}{lean}
...
\begin{warningbox}[title={Why not a real proof here?}]
The earlier edition printed a tactic proof at this spot that did not compile:
\li{unfold solve} yields a single goal (no case bullets to split on), the
``case~1'' \li{intro} acted on a \li{match} without a \li{split}, and four of
the five helper lemmas it invoked (\li{domain\_empty}, \li{extractBuild\_feasible},
\li{bb\_invariant\_at\_termination}, \li{queue\_empty}) were never defined.  We
replace it with an explicit \li{sorry}.  A genuine proof would \li{split} on the
result of \li{solve}, and in the \li{.optimal} arm chain
\li{propagation\_preserves\_feasible} (every feasible build survives pruning),
\li{lp\_bound\_valid} and \li{bb\_step\_preserves} (nothing better hides in a
pruned subtree), and \li{infusion\_dp\_optimal} (the leaf's infusions are
optimal); the \li{.infeasible} arm follows from
\li{propagation\_preserves\_feasible} plus the leaf floor-check in
\li{branchAndBound}.  Each piece is a substantial formalisation task.
\end{warningbox}
...
This eliminates the $O(14 \times 40)$ cost of copying domain arrays at each
...
The \li{then} branch adds to \li{acc}.  The earlier edition wrote
\li{conv.percentage * \ldots} \emph{without} \li{acc +}, discarding the
accumulator---so with two conversions into Power only the \emph{last} counted
(two 10\% conversions yielding $4.0$ instead of the correct $9.0$).
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- What-If Mode}]
\begin{minted}{lean}
...
The search space shrinks dramatically: if 6 of the 14 slots are fixed, the
remaining space is $40^{8} \approx 6.6 \times 10^{12}$ instead of $40^{14}
\approx 6.9 \times 10^{22}$---and propagation typically prunes this much further.
...
\begin{leanbox}[title={Lean 4 (Mathlib) --- Pareto Frontier Mode}]
\begin{minted}{lean}
...
Two fixes: \li{Float.toNat} does not exist in v4.31 (route through
\li{.toUInt64.toNat}), and the earlier edition's closing brace \li{\}} was
\emph{inside} the trailing comment, so the record update never closed.  For
\li{wMissPerStat} to actually steer the sweep, the scalarised \li{solve} and
\li{totalPenalty} must read it in place of the global \li{wMiss}---a small
extension left to the reader.
...
this book uses.  Each entry says what it does, shows a small self-contained
example (every example here is machine-checked---see the \li{Primer} namespace of
\li{OptimizationVerification.lean}), and, where it illuminates, shows the
lower-level term the tactic is sugar for.  Tactics are not magic: they are
programs that build proof terms.

\section{Declarations}

\subsection*{\li{inductive} --- new types}
An \li{inductive} declaration creates a type from a list of \emph{constructors}
(the only ways to build its values).  Lean auto-generates an induction principle
(the \emph{recursor}).
...
\li{deriving Repr} synthesises a printer (so \li{\#eval} can display values);
\li{deriving DecidableEq} synthesises a decidable \li{=}.

\subsection*{\li{def} and pattern matching}
A \li{def} introduces a function or value, usually by \emph{pattern matching}:
one equation per constructor.  Recursion is allowed when an argument
structurally shrinks; otherwise you must write \li{partial def} (as the solver's
search loops do) and forfeit the ability to reason about the function in proofs.
...
The leading dot in \li{.red} means ``the constructor of the type being matched''.

\subsection*{\li{structure} and \li{instance}}
A \li{structure} is a record; a \emph{type class} is an interface and an
\li{instance} registers an implementation.  The anonymous constructor
\li{⟨\ldots⟩} builds a structure (and also proves \li{∧} and \li{∃}: \li{⟨w, h⟩}
supplies a witness and a proof).

\subsection*{\li{theorem}, \li{example}, \li{Prop}}
In Lean, propositions are types (of sort \li{Prop}) and proofs are terms: to
prove $P$ is to build a term of type $P$.  A \li{theorem} is a named proof; an
\li{example} is anonymous.  A proof is either a direct term or a
\emph{tactic block} introduced by \li{by}.
...
If the goal is \li{p → q} (or \li{∀ x, P x}), \li{intro hp} moves the antecedent
into context, leaving goal \li{q}.  It is the tactic form of \li{fun hp => …}.

\subsection*{\li{exact} --- supply the proof term}
\li{exact h} closes the goal with \li{h}, which must match exactly.

\subsection*{\li{constructor} --- build a structure/conjunction goal}
When the goal is a structure or \li{∧}, \li{constructor} splits it into one
subgoal per field.  The two bullets \li{·} below discharge each conjunct.
...
\subsection*{\li{obtain} --- destructure a hypothesis}
\li{obtain ⟨n, hn⟩ := h} takes apart an \li{∃} or \li{∧}.  It desugars to
\li{Exists.elim} (or a \li{match}):
...
\subsection*{\li{unfold} --- expand a definition}
\li{unfold f} replaces \li{f} by its definition in the goal (then usually
followed by \li{rfl} or \li{simp}).
...
Proves \li{a = a} after both sides compute to the same normal form; definitional
unfolding is free, so \li{2 + 3 = 5} is \li{rfl}.

\subsection*{\li{simp} --- the simplifier}
Rewrites with every \li{@[simp]} rule (plus any you pass in brackets) until
nothing changes, then tries to close the goal.  \li{simpa using h} simplifies
both the goal and \li{h} and closes with it (used in the \li{Decidable} instance
for dominance).
...
\subsection*{\li{omega} --- linear arithmetic over \li{Nat}/\li{Int}}
A decision procedure for linear integer/natural arithmetic (with $+$, $-$,
$\leq$, $<$, and multiplication by literals).  It closes goals like the ones in
this book's Pareto and LP-bound sanity checks.
...
\subsection*{\li{split} --- case on an \li{if}/\li{match} in the goal}
When the goal contains \li{if c then … else …} (or a \li{match}), \li{split}
creates one subgoal per branch, adding the branch condition as a hypothesis.
...
\subsection*{\li{induction} / \li{cases} --- and their recursor desugaring}
\li{induction n with} splits into one case per constructor, giving the step case
an induction hypothesis.  Under the hood there is no primitive ``induction'':
every \li{inductive} type \li{T} comes with a \emph{recursor} \li{T.rec} (a
function stating the induction principle), and the tactic merely applies it.
\li{cases} is the same without the induction hypothesis.
...
\section{\texorpdfstring{\li{sorry}}{sorry} --- ``unproven''}

\begin{warningbox}
\li{sorry} closes \emph{any} goal, unconditionally, by \emph{admitting} it
without proof.  It is a placeholder, not a proof: Lean accepts the file but emits
the warning \li{declaration uses 'sorry'}, and anything built on a \li{sorry} is
logically unfounded.  In this book, a \li{sorry} always marks a
\textbf{specification}---a precisely stated theorem whose proof is left as an
extended exercise.  We never present a \li{sorry}-backed theorem as if it were
machine-checked.  Whenever you see \li{sorry}, read it as ``this is asserted, not
established''.
\end{warningbox}
...
\li{Finset} (finite sets), \li{SemilatticeSup}/\li{⊔} (join-semilattices), and
\li{BinaryHeap} (a binary heap, from Batteries) are \textbf{not} core Lean---they
require \li{import Mathlib} (Batteries ships with it).  See
Section~\ref{sec:mathlib-setup} for project setup.  \li{decidable\_of\_iff},
\li{List.all\_eq\_true}, and \li{List.any\_eq\_true} (used to make dominance
decidable) are core Lean.
