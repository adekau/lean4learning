...
Comfort with Lean\,4 syntax: defining inductive types, writing functions,
using tactics (\lc{intro}, \lc{rfl}, \lc{simp}, \lc{omega}, \lc{cases},
\lc{induction}). No prior abstract algebra.

\paragraph{Lean\,4 setup.}
All code in this book is written for vanilla Lean\,4 (no Mathlib dependency).
We build every algebraic structure from scratch so you see exactly what's
happening inside the typeclasses.
...
theorem GroupHom.map_one [Group G] [Group H] (φ : GroupHom G H) :
  ...
  -- Multiplying both sides by (φ.toFun 1)⁻¹ gives the result
...
\begin{defn}{Galois Connection}{galois-connection}
Let $(P, \leq_P)$ and $(Q, \leq_Q)$ be posets. A \textbf{Galois connection}
between $P$ and $Q$ is a pair of maps
$\ell : P \to Q$ and $u : Q \to P$ such that:
\[
  \ell(p) \leq_Q q \quad \iff \quad p \leq_P u(q)
  \qquad \text{for all } p \in P, \; q \in Q.
\]
We call $\ell$ the \textbf{lower adjoint} and $u$ the \textbf{upper adjoint}.
\end{defn}
...
Let $(\ell, u)$ be a Galois connection between $P$ and $Q$. Then:
\begin{enumerate}[nosep, label=(\roman*)]
\item Both $\ell$ and $u$ are \textbf{order-reversing}:
  if $p_1 \leq p_2$ then $\ell(p_2) \leq \ell(p_1)$, and similarly for $u$.
\item \textbf{Unit:} $p \leq u(\ell(p))$ for all $p \in P$.
\item \textbf{Counit:} $\ell(u(q)) \leq q$ for all $q \in Q$.
...
\begin{proof}
All follow from the adjunction condition.
(i)~If $p_1 \leq p_2$, then $p_2 \leq u(\ell(p_2))$ by (ii), so
$p_1 \leq u(\ell(p_2))$, so $\ell(p_2) \leq \ell(p_1)$ by adjunction.
(Wait---this proves $\ell$ is order-\emph{reversing} only if the Galois
connection is \emph{antitone}. For the standard definition with an antitone
connection, both maps reverse order.)
\end{proof}
...
and others with \emph{both maps order-reversing} (the ``antitone'' variant).
The Galois correspondence in algebra uses the antitone variant: bigger fields
$\leftrightarrow$ smaller groups. We follow the antitone convention.
\end{warning}
...
\begin{tikzcd}[row sep=0.5cm]
  ...
  \arrow[r, shift left, "\Fix"]
  &
  \{\text{subgroups } H \leq G\}
  \arrow[l, shift left, "\Inv"]
\end{tikzcd}
\]
given by:
\begin{align*}
  \Fix(K) &= \Gal(E/K) = \{\sigma \in G : \sigma(x) = x \text{ for all } x \in K\} \\
  \Inv(H) &= E^H = \{x \in E : \sigma(x) = x \text{ for all } \sigma \in H\}
\end{align*}
Moreover:
\begin{enumerate}[nosep, label=(\roman*)]
\item $\Fix$ and $\Inv$ are mutual inverses: $\Inv(\Fix(K)) = K$ and
  $\Fix(\Inv(H)) = H$.
\item The correspondence \textbf{reverses order}: $K_1 \subseteq K_2$
  iff $\Fix(K_2) \leq \Fix(K_1)$.
\item \textbf{Degrees match indices:} $[E:K] = |\Fix(K)|$ and
  $[K:F] = [G : \Fix(K)]$.
\item $K/F$ is a normal (Galois) extension iff $\Fix(K) \normal G$.
  In that case, $\Gal(K/F) \iso G / \Fix(K)$.
\end{enumerate}
\end{thm}
...
\begin{tikzpicture}[
  ...
  % Arrow
  \draw[<->, very thick, accentpurple] (3.5, -1.5) -- node[above] {$\Fix / \Inv$} (5.5, -1.5);
...
$D_4$ has 10 subgroups. By the fundamental theorem, there are exactly 10
intermediate fields. Here are the two lattices, side by side:

\begin{center}
\small
\begin{tikzpicture}[
    fld/.style={draw, rounded corners, fill=lightblue!80, font=\footnotesize,
      minimum width=1.6cm, inner sep=2pt},
    grp/.style={draw, rounded corners, fill=lightgreen!80, font=\footnotesize,
      minimum width=1.6cm, inner sep=2pt},
    scale=0.9,
  ]
  % FIELDS
  \node[fld] (E) at (0, 0) {$E$};
  \node[fld] (Qi) at (-2.5, -1.5) {$\Q(\sqrt[4]{2},i)$};
  \node[fld] (Q4) at (-0.8, -1.5) {$\Q(\sqrt[4]{2})$};
  \node[fld] (Qi4) at (0.8, -1.5) {$\Q(i\sqrt[4]{2})$};
  \node[fld] (Qsqrt2) at (-2, -3) {$\Q(\sqrt{2},i)$};
  \node[fld] (Qsqrt2b) at (0, -3) {$\Q(\sqrt{2})$};
  \node[fld] (Qisqrt2) at (2, -3) {$\Q(i\sqrt{2})$};
  \node[fld] (Qi2) at (-1, -4.5) {$\Q(i)$};
  \node[fld] (QQ) at (0, -6) {$\Q$};

  \draw (E) -- (Qi) -- (Qsqrt2) -- (Qi2) -- (QQ);
  \draw (E) -- (Q4) -- (Qsqrt2b) -- (QQ);
  \draw (E) -- (Qi4) -- (Qisqrt2) -- (QQ);
  \draw (Qsqrt2) -- (QQ);

  \node[above=0.2cm of E, font=\footnotesize\bfseries\color{deepblue}] {Fields};

  % Arrow
  \node at (4, -3) {\Large $\longleftrightarrow$};

  % SUBGROUPS (reversed!)
  \node[grp] (1) at (8, 0) {$\{e\}$};
  \node[grp] (t) at (5.5, -1.5) {$\langle\tau\rangle$};
  \node[grp] (r2) at (7.2, -1.5) {$\langle\rho^2\rangle$};
  \node[grp] (rt) at (8.8, -1.5) {$\langle\rho^2\tau\rangle$};
  \node[grp] (r2t) at (6, -3) {$\langle\rho^2,\tau\rangle$};
  \node[grp] (r2b) at (8, -3) {$\langle\rho^2\rangle$};
  \node[grp] (r2rt) at (10, -3) {$\langle\rho^2,\rho\tau\rangle$};
  \node[grp] (r) at (9, -4.5) {$\langle\rho\rangle$};
  \node[grp] (D4) at (8, -6) {$D_4$};
  ...
  \node[above=0.2cm of 1, font=\footnotesize\bfseries\color{accentgreen}] {Subgroups};
\end{tikzpicture}
\end{center}
...
original example: $\Fix$ and $\Inv$ form an antitone Galois connection between
the lattice of intermediate fields and the lattice of subgroups.
...
\chapter*{Appendix: Lean\,4 Typeclass Hierarchy}
\addcontentsline{toc}{chapter}{Appendix: Lean 4 Typeclass Hierarchy}
