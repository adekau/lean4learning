...
because of a race condition. The Ariane~5 rocket exploded because of a
concurrent sensor processing fault. These are not hypothetical risks.
...
All code is vanilla Lean\,4, no Mathlib. We build every structure from
scratch.
...
\item \textsc{Comm:} If $P \xrightarrow{a} P'$ and $Q \xrightarrow{\bar{a}} Q'$
  then $P \Par Q \xrightarrow{\tau} P' \Par Q'$
...
derivation using \lc{CCSStep} constructors.
\end{exercisebox}
...
Both $A$ and $B$ want to send first. Under restriction, neither output is
visible externally, and neither can synchronize because $A$ outputs on $b$
while $B$ outputs on $a$---but $A$ is waiting for input on $a$ (which $B$
hasn't sent yet) and $B$ is waiting for input on $b$ (which $A$ hasn't sent
yet). Deadlock.
...
Function application creates a fresh channel $v$, evaluates the function,
receives its body, evaluates the argument, and forwards the result.
...
\stepcounter{exercise}
Encode the $\lambda$-term $(\lambda x. x)\; y$ (identity applied to $y$)
using Milner's encoding. Trace the reduction steps and verify that the
result is $y$.
\end{exercisebox}
...
\stepcounter{exercise}
Prove in Lean that the worker pool protocol satisfies the deadlock-freedom
property. \emph{Hint:} show that in every reachable state, at least one
communication rule (from the $\pi$-calculus operational semantics) is applicable.
\end{exercisebox}
...
\item \textbf{Galois theory book:} The algebraic laws of parallel composition
  ...
  of field arithmetic. The subgroup lattice ↔ intermediate field lattice
  correspondence is structurally parallel to the specification ↔
  implementation correspondence in refinement theory.
\end{itemize}
